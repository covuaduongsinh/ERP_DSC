// ============================================================
// @vierp/events - Event Flow: CoVua → Accounting
// covua.payment.received → JournalEntryPosted (ghi nhận doanh thu học phí)
// Nợ Tiền (1111) / Có Doanh thu dịch vụ (5113) + bán sách (5111) + khác (711)
// ============================================================

import type { TuitionPaymentReceived } from '../schemas/covua.events';
...
/**
 * Map covua.payment.received → JournalEntryPosted.
...
export async function mapTuitionPaymentToJournalEntry(
  ...
  const journalNumber = `JE-COVUA-${payload.code || payload.paymentId}`;
  ...
  return {
    ...
    description: `Bút toán thu học phí CoVua — ${payload.code || payload.paymentId}`,
    sourceEvent: 'payment',
    ...
    lines,
    notes: `Tự sinh từ covua.payment.received. Cơ sở: ${payload.location || 'n/a'}.`,
    postedBy: 'system',
  };
}

/** Metadata flow CoVua → Accounting (đăng ký trong FlowRegistry). */
export const CoVuaToAccountingFlow = {
