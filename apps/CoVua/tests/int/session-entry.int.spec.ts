...
function makePayload(classDoc: Record<string, unknown> = { id: 5, location: 2, lichHoc: [] }) {
  ...
  const create = async ({ collection, data }: { collection: string; data: Record<string, unknown> }) => {
    if (collection === 'class-sessions') {
  ...
  const update = async ({ collection, id, data }: { collection: string; id: number; data: Record<string, unknown> }) => {
...
describe('saveSessionEntry — nội dung buổi + upsert điểm danh 🔒', () => {
  it('non-staff ⇒ forbidden; classId sai ⇒ invalid_input', async () => {
    ...
    expect((await saveSessionEntry(payload, null, { classId: 5, date: '2026-06-04', session: {}, entries: [] })).ok).toBe(false);
    expect((await saveSessionEntry(payload, admin, { classId: 0, date: '2026-06-04', session: {}, entries: [] })).ok).toBe(false);
  });
  ...
  it('lưu mới ⇒ tạo buổi (da_day) + attendance gắn session', async () => {
    ...
    expect(sessions[0]).toMatchObject({ trangThai: 'da_day', kienThucMoi: 'Chiếu hết', coachThucTe: 10 });
    expect(attendance).toHaveLength(1);
    expect(attendance[0]).toMatchObject({ student: 1001, status: 'co_mat', lop: 5, yThuc: 8, session: sessions[0].id });
  });

  it('lưu lại cùng ngày ⇒ cập nhật, KHÔNG tạo trùng (idempotent)', async () => {
    const { payload, attendance } = makePayload();
    const base = { classId: 5, date: '2026-06-04', session: {}, entries: [{ studentId: 1001, status: 'co_mat' as const }] };
    await saveSessionEntry(payload, admin, base);
    const res2 = await saveSessionEntry(payload, admin, { ...base, entries: [{ studentId: 1001, status: 'vang' as const }] });
...
describe('setSessionCancelled + createMakeupSession 🔒', () => {
  it('hủy buổi ⇒ trangThai huy + ghiChuBuoi', async () => {
    ...
    const res = await setSessionCancelled(payload, admin, { classId: 5, date: '2026-06-04', huy: true, reason: 'Nghỉ lễ' });
  ...
  it('mở lại buổi ⇒ trangThai da_day', async () => {
    const { payload, sessions } = makePayload();
    await setSessionCancelled(payload, admin, { classId: 5, date: '2026-06-04', huy: true, reason: 'x' });
  ...
  it('tạo buổi bù ⇒ trangThai bu + buoiGoc', async () => {
    const { payload, sessions } = makePayload();
    const res = await createMakeupSession(payload, admin, { classId: 5, originalSessionId: 99, date: '2026-06-11' });
    expect(res.ok).toBe(true);
  ...
  it('non-staff ⇒ forbidden', async () => {
    const { payload } = makePayload();
    expect((await setSessionCancelled(payload, null, { classId: 5, date: '2026-06-04', huy: true })).ok).toBe(false);
    expect((await createMakeupSession(payload, null, { classId: 5, originalSessionId: 1, date: '2026-06-11' })).ok).toBe(false);
  });
});
