...
const isFilteredByStudentIn = (allowed: number[]) => (v: unknown) => {
  ...
  if (!Array.isArray(studentIn)) return { ok: false, detail: `no student.in in filter: ${JSON.stringify(v)}` };
  const same =
    studentIn.length === allowed.length &&
    studentIn.every((id) => allowed.includes(id));
  return {
    ok: same,
    detail: same
...
const isFilteredByIdIn = (allowed: number[]) => (v: unknown) => {
  ...
  const same =
    idIn.length === allowed.length && idIn.every((id) => allowed.includes(id));
  return { ok: same, detail: same ? `id.in = [${idIn.join(',')}]` : `mismatch: got [${idIn.join(',')}]` };
};
...
async function main() {
  ...
  await expect('Students.read as staff → full access', readStudentForParent, asReq(staff), isStrictlyTrue);

  // 2. Students.read — anon bị từ chối
  await expect('Students.read as anonymous → denied', readStudentForParent, asReq(anon), isStrictlyFalse);
  ...
  await expect(
    ...
    (v) => {
      const ids = isWhere(v)
        ? (((v.and as Where[] | undefined)?.[0] ?? v) as { student?: { in?: number[] } })
            .student?.in
        : undefined;
  ...
  for (const [name, coll] of [
    ...
    if (!readFn) {
      ...
      await expect(
        `${name}.read as staff → full access`,
        readFn,
        asReq(staff),
        isStrictlyTrue,
      );
    ...
    for (const op of ['create', 'update', 'delete'] as const) {
      ...
      await expect(`${name}.${op} as Parent A → denied (staff only)`, fn, asReq(parentA), isStrictlyFalse);
  ...
  for (const op of ['create', 'update', 'delete'] as const) {
    ...
    await expect(
      `TuitionCycles.${op} as staff → allowed`,
      fn,
      asReq(staff),
      isStrictlyTrue,
    );
    await expect(
      `TuitionCycles.${op} as anonymous → denied`,
      fn,
      asReq(anon),
      isStrictlyFalse,
    );
  ...
  for (const [name, coll] of [
    ...
    for (const op of ['create', 'update', 'delete'] as const) {
      ...
      await expect(
        `${name}.${op} as Parent A → denied`,
        fn,
        asReq(parentA),
        isStrictlyFalse,
      );
      await expect(
        `${name}.${op} as anonymous → denied`,
        fn,
        asReq(anon),
        isStrictlyFalse,
      );
  ...
  for (const op of ['create', 'read', 'update', 'delete'] as const) {
    ...
    for (const [who, ctx] of [
      ...
      await expect(
        `OtpCodes.${op} as ${who} → denied (sealed)`,
        fn,
        asReq(ctx),
        isStrictlyFalse,
      );
