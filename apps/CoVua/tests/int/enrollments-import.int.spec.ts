...
const STUDENTS = [{ id: 1001, fullName: 'Nguyễn Văn An' }, { id: 1002, fullName: 'Trần Bảo Minh' }];
const CLASSES = [{ id: 5, title: 'Lớp A' }, { id: 6, title: 'Lớp B' }];
...
describe('importEnrollments — match + idempotent (student,class) 🔒', () => {
  ...
  it('một ô NHIỀU lớp (ngăn ;) ⇒ tạo nhiều ghi danh cho 1 HV', async () => {
    ...
    const classesEnrolled = create.mock.calls.map((c) => (c[0] as { data: { class: number } }).data.class).sort();
