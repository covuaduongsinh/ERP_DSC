...
describe('validatePaymentInput', () => {
  ...
  it('không có khoản tiền > 0 ⇒ lỗi (chống phiếu rỗng)', () => {
    ...
    expect(validatePaymentInput({ student: 5, ngayNop: '2026-06-02', soBuoiNop: 10 }).ok).toBe(false);
  });
  ...
  it('tiền sách/mua khác cũng tính là khoản tiền hợp lệ', () => {
    expect(validatePaymentInput({ student: 5, ngayNop: '2026-06-02', tienSach: 200_000 }).ok).toBe(true);
    expect(validatePaymentInput({ student: 5, ngayNop: '2026-06-02', muaKhac: 50_000 }).ok).toBe(true);
  });
