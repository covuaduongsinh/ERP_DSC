...
const CLASS_CONTENT = { sachDangHoc: 'Sách lớp', kienThucMoi: 'KT lớp', giaoBTVN: null, khBuoiSau: null };
...
const ENROLLMENTS = [
  ...
  { student: 1004, class: LOP, dangHoc: false, ngayBatDau: '2026-01-01', ngayKetThuc: '2026-06-05' }, // nghỉ trước
];
...
function makeFake(session: Record<string, unknown> = SESSION_DOC) {
  ...
  const find = vi.fn(async ({ collection, where }: { collection: string; where?: Record<string, unknown> }) => {
    if (collection === 'enrollments') {
      ...
      const batDauLte = (((and[1] as { or?: Array<Record<string, unknown>> })?.or?.[1]) as { ngayBatDau?: { less_than_equal?: string } })?.ngayBatDau?.less_than_equal;
      const ketThucGte = (((and[2] as { or?: Array<Record<string, unknown>> })?.or?.[2]) as { ngayKetThuc?: { greater_than_equal?: string } })?.ngayKetThuc?.greater_than_equal;
      const docs = ENROLLMENTS.filter((e) => {
        if (e.class !== classId) return false;
        if (e.ngayBatDau != null && batDauLte != null && t(e.ngayBatDau)! > t(batDauLte)!) return false;
        const keepByEnd = e.dangHoc === true || e.ngayKetThuc == null || (ketThucGte != null && t(e.ngayKetThuc)! >= t(ketThucGte)!);
        return keepByEnd;
      });
    ...
    if (collection === 'session-student-plans') {
      const w = (where ?? {}) as { session?: { equals?: number }; khoaKeHoach?: { equals?: string } };
      let docs = plans.slice();
      if (w.khoaKeHoach?.equals != null) docs = docs.filter((p) => p.khoaKeHoach === w.khoaKeHoach!.equals);
      else if (w.session?.equals != null) docs = docs.filter((p) => p.session === w.session!.equals);
      return { docs, totalDocs: docs.length };
    }
  ...
  const create = vi.fn(async ({ collection, data }: { collection: string; data: Record<string, unknown> }) => {
    ...
    if (data.khoaKeHoach && plans.some((p) => p.khoaKeHoach === data.khoaKeHoach)) {
      throw new Error('duplicate key value violates unique constraint "session_student_plans_khoa_ke_hoach_idx"');
    }
...
describe('updateStudentPlan — ghi đè idempotent + ranh giới tiền 🔒', () => {
  it('ghi đè 1 HV ⇒ effective = override; gọi 2 lần ⇒ 1 dòng; KHÔNG đụng attendance', async () => {
    ...
    const a = await updateStudentPlan(payload, manager, { sessionId: SESSION_ID, studentId: 1001, patch: { sachDangHoc: 'Sách An' } });
    expect(a.ok).toBe(true);
    const b = await updateStudentPlan(payload, manager, { sessionId: SESSION_ID, studentId: 1001, patch: { sachDangHoc: 'Sách An (sửa)' } });
  ...
  it('HV ngoài roster (vào sau) ⇒ forbidden, không ghi', async () => {
    ...
    const res = await updateStudentPlan(payload, manager, { sessionId: SESSION_ID, studentId: 1003, patch: { sachDangHoc: 'x' } });
  ...
  it('non-staff ⇒ forbidden', async () => {
    const { payload } = makeFake();
    expect((await updateStudentPlan(payload, parentA, { sessionId: SESSION_ID, studentId: 1001, patch: { sachDangHoc: 'x' } })).ok).toBe(false);
  });
});
...
describe('applyClassPlanToAll — vật chất hóa nội dung cả lớp 🔒', () => {
  it('ghi 1 dòng/HV roster với nội dung cả lớp; KHÔNG đụng attendance', async () => {
    ...
    expect(plans.every((p) => p.sachDangHoc === 'Sách lớp' && p.kienThucMoi === 'KT lớp')).toBe(true);
    expect(attendanceOps).toEqual({ create: 0, update: 0, delete: 0 });
  });
});
