...
const coachKL = { id: 10, collection: 'users', role: 'coach', location: KIM_LIEN } as unknown as User;
...
function makeFakePayload(
  ...
  const create = vi.fn(async ({ collection, data }: { collection: string; data: Record<string, unknown> }) => {
    ...
    if (data.khoaBuoi && sessions.some((s) => s.khoaBuoi === data.khoaBuoi)) {
      throw new Error('duplicate key value violates unique constraint "class_sessions_khoa_buoi_idx"');
    }
    const doc = { id: nextId++, ...data };
    sessions.push(doc);
    return doc;
  ...
  const find = vi.fn(async ({ collection, where }: { collection: string; where?: Record<string, unknown> }) => {
    ...
    if (w.khoaBuoi?.equals != null) {
      ...
      for (const cond of w.and) {
        const c = cond as Record<string, { equals?: unknown; greater_than_equal?: string; less_than_equal?: string }>;
        if (c.lop?.equals != null) docs = docs.filter((s) => s.lop === c.lop!.equals);
        if (c.date?.greater_than_equal)
          docs = docs.filter((s) => new Date(s.date as string).getTime() >= new Date(c.date!.greater_than_equal!).getTime());
        if (c.date?.less_than_equal)
          docs = docs.filter((s) => new Date(s.date as string).getTime() <= new Date(c.date!.less_than_equal!).getTime());
      }
      docs = docs.sort((a, b) => new Date(a.date as string).getTime() - new Date(b.date as string).getTime());
    }
    return { docs, totalDocs: docs.length };
  });
...
describe('loadPlannedSessions — nạp buổi để sửa/hủy/bù', () => {
  it('trả về các buổi của lớp trong khoảng, sort theo ngày', async () => {
    ...
    expect(res.rows.map((r) => r.date)).toEqual(['2026-06-01', '2026-06-03', '2026-06-08', '2026-06-10']);
...
describe('cancelPlannedSession / createPlannedMakeup — planner-gate 🔒', () => {
  it('hủy buổi: planner ⇒ trangThai=huy + lý do; coach ⇒ forbidden', async () => {
    ...
    const denied = await cancelPlannedSession(payload, coachKL, { classId: LOP, date: '2026-06-01', reason: 'x' });
    expect(denied.ok).toBe(false);

    const res = await cancelPlannedSession(payload, manager, { classId: LOP, date: '2026-06-01', reason: 'Nghỉ lễ' });
    expect(res.ok).toBe(true);
