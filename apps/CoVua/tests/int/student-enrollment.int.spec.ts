...
describe('findTimeConflicts — trùng giờ cho 1 HV (thuần)', () => {
  const mk = (id: number, title: string, slots: ClassSlots['slots']): ClassSlots => ({ id, title, slots });

  it('hai lớp cùng thứ + giao giờ ⇒ 1 cặp trùng (1 lần/cặp)', () => {
    const a = mk(1, 'A', [{ thu: 't2', gioBatDau: '17:00', gioKetThuc: '18:30' }, { thu: 't4', gioBatDau: '17:00', gioKetThuc: '18:30' }]);
    const b = mk(2, 'B', [{ thu: 't2', gioBatDau: '18:00', gioKetThuc: '19:00' }, { thu: 't4', gioBatDau: '18:00', gioKetThuc: '19:00' }]);
  ...
  it('rỗng / 1 lớp ⇒ không trùng', () => {
    ...
    expect(findTimeConflicts([mk(1, 'A', [{ thu: 't2', gioBatDau: '17:00', gioKetThuc: '18:30' }])])).toEqual([]);
  });
});
