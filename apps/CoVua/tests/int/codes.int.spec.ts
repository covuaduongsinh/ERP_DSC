...
describe('rollover — reset seq theo kỳ', () => {
  ...
  it('Payments: 06→07 reset seq theo tháng', async () => {
    const pool = makeFakePool();
    const jun = { ngayNop: '2026-06-30T00:00:00.000Z', coSo: 'vinh_phuc' } as Record<string, unknown>;
    const jul = { ngayNop: '2026-07-01T00:00:00.000Z', coSo: 'vinh_phuc' } as Record<string, unknown>;
...
describe('bất biến — lockCodeHook chặn sửa code mọi vai trò', () => {
  ...
  const run = (data: Record<string, unknown> | undefined, originalDoc: Record<string, unknown>, operation = 'update') =>
    lockCodeHook({
      data,
      operation,
      originalDoc,
...
describe('makeCodeHook — không ghi đè mã đã có', () => {
  it('data.code đã có (migration/khôi phục) → giữ nguyên, không cấp số', async () => {
    const pool = makeFakePool();
    const data = { createdAt: '2026-01-01T00:00:00.000Z', code: 'HV-26-0500' } as Record<string, unknown>;
...
describe('🔒 firewall học phí — phụ huynh đọc Payments scoped + field ẩn', () => {
  ...
  it('config Payments: field nhạy cảm gắn read=staffOnlyFieldRead; code & ngayNop & tiền KHÔNG ẩn', () => {
    ...
    const hidden = ['tuitionCycle', 'hp1Buoi', 'soBuoiNop', 'coSo', 'tinhTrang', 'anhBill', 'ghiChu'];
