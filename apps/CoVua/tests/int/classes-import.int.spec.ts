...
function makePayload(opts: { locations?: Doc[]; coaches?: Doc[]; classes?: Doc[] }) {
  ...
  const find = vi.fn(async ({ collection, where }: { collection: string; where?: any }) => {
    if (collection === 'locations') return { docs: opts.locations ?? [], totalDocs: (opts.locations ?? []).length };
    if (collection === 'coaches') return { docs: opts.coaches ?? [], totalDocs: (opts.coaches ?? []).length };
...
describe('importClasses — idempotent theo title 🔒', () => {
  it('lớp mới hợp lệ ⇒ created, khớp cơ sở/cấp/GV/lịch', async () => {
    ...
    const r = await importClasses(payload, 'f.csv', [
      row({ ten_lop: 'Lớp Tốt 1 - Kim Liên', cap_do: 'Tốt', nhom_tuoi: 'Cấp 1 - Cấp 2', co_so: 'Kim Liên', gv: 'Quyên', si_so_toi_da: '15', lich_hoc: 'T2 17:00-18:30 A1' }),
    ]);
    expect(r.created).toBe(1);
    expect(r.errors).toBe(0);
    const arg = create.mock.calls[0][0] as { data: Record<string, unknown> };
    expect(arg.data).toMatchObject({ title: 'Lớp Tốt 1 - Kim Liên', level: ['tot'], ageGroup: ['cap_1_cap_2'], location: 2, coach: 10, siSoToiDa: 15 });
  ...
  it('cap_do gồm nhiều cấp ⇒ level mảng (tuyến tự suy qua hook collection)', async () => {
    const { payload, create } = makePayload({ locations: LOCS, coaches: COACHES });
    const r = await importClasses(payload, 'f.csv', [
      row({ ten_lop: 'Lớp Gộp', cap_do: 'Tốt, Hậu', nhom_tuoi: 'Cấp 1 - Cấp 2', co_so: 'Kim Liên' }),
    ]);
    expect(r.created).toBe(1);
    expect(r.errors).toBe(0);
    const arg = create.mock.calls[0][0] as { data: Record<string, unknown> };
  ...
  it('nhom_tuoi gồm cả hai ⇒ ageGroup mảng 2 nhóm', async () => {
    const { payload, create } = makePayload({ locations: LOCS, coaches: COACHES });
    const r = await importClasses(payload, 'f.csv', [
      row({ ten_lop: 'Lớp Chung', cap_do: 'Tốt', nhom_tuoi: 'Mầm non, Cấp 1 - Cấp 2', co_so: 'Kim Liên' }),
    ]);
    expect(r.created).toBe(1);
    expect(r.errors).toBe(0);
    const arg = create.mock.calls[0][0] as { data: Record<string, unknown> };
  ...
  it('GV không khớp tenTat ⇒ vẫn created kèm cảnh báo', async () => {
    const { payload, create } = makePayload({ locations: LOCS, coaches: COACHES });
    const r = await importClasses(payload, 'f.csv', [
      row({ ten_lop: 'Lớp Z', cap_do: 'Mã', nhom_tuoi: 'Mầm non', co_so: 'Vĩnh Phúc', gv: 'KhôngCó' }),
    ]);
    expect(r.created).toBe(1);
  ...
  it('title đã tồn tại ⇒ updated (không create)', async () => {
    ...
    const r = await importClasses(payload, 'f.csv', [
      row({ ten_lop: 'Lớp Tốt 1 - Kim Liên', cap_do: 'Tốt', nhom_tuoi: 'Cấp 1', co_so: 'Kim Liên' }),
    ]);
