...
describe('ensureSession — idempotent qua khoaBuoi 🔒', () => {
  ...
  it('gọi 2 lần cùng (lop, ngày) ⇒ CHỈ 1 bản ghi (idempotent)', async () => {
    ...
    const a = (await ensureSession(payload, admin, { lopId: 5001, date })) as unknown as { id: number };
    const b = (await ensureSession(payload, admin, { lopId: 5001, date })) as unknown as { id: number };
