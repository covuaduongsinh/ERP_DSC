...
export async function fetchActiveTuitionCycles(
  parentUser: Parent,
): Promise<TuitionCycle[]> {
  const payload = await getPayloadClient();
  const result = await payload.find({
...
export function mapClassTitleByStudent(
...
export function mapTuitionByStudent(
  cycles: TuitionCycle[],
): Map<number, TuitionCycle> {
  const map = new Map<number, TuitionCycle>();
  for (const cycle of cycles) {
    const studentId =
      typeof cycle.student === 'number' ? cycle.student : cycle.student.id;
...
export async function fetchPendingRenewalStudentIds(
  parentUser: Parent,
): Promise<Set<number>> {
  const payload = await getPayloadClient();
  const result = await payload.find({
  ...
  for (const doc of result.docs) {
    const studentId =
      typeof doc.student === 'number' ? doc.student : doc.student.id;
    ids.add(studentId);
  }
  return ids;
}
...
export async function fetchStudentProgress(
  ...
  const [reportsResult, attendanceResult, tuitionResult, enrollmentResult] =
    await Promise.all([
    payload.find({
