 ...
 * "Tuyển sinh" được CHỐT — đường handoff CRM → CoVua.
 *
 * Khác `convertLeadToParentStudent` (đầu vào là `leadId` của CoVua): ở kiến trúc
...
export interface AdmissionUpsertInput {
  ...
  /** id classes (CoVua) — có thì tạo Enrollment. */
  classId?: number | null
...
export async function upsertParentStudentEnrollment(
  ...
  try {
    ...
    payload.logger.error({ err, crmDealId: input.crmDealId }, '[admission] lỗi handoff CRM→CoVua')
    return {
      ok: false,
      error: 'server',
