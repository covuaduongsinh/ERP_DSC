...
export async function importEnrollments(
  ...
  const classes = await payload.find({ collection: 'classes', limit: 2000, overrideAccess: true, depth: 0 });
  ...
  for (let page = 1; ; page += 1) {
    ...
    for (const d of batch.docs) {
      const sid = typeof d.student === 'number' ? d.student : (d.student as { id?: number } | null)?.id;
      const cid = typeof d.class === 'number' ? d.class : (d.class as { id?: number } | null)?.id;
      if (typeof sid === 'number' && typeof cid === 'number') existingMap.set(`${sid}|${cid}`, d.id as number);
    }
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const lookup = await lookupStudent({ payload, studentCode: maHocVien, fullName: tenHocVien, parentPhone: sdtPh });
    if (!lookup.ok) {
    ...
    for (const lopName of lopNames) {
      ...
      if (seenInFile.has(dedupeKey)) {
        record({ row: rowNumber, status: 'skipped', label, message: 'Trùng (học viên + lớp) — bỏ qua.' });
        continue;
      }
      ...
      try {
        ...
        if (existingId != null) {
          if (!dryRun) {
            await payload.update({ collection: 'enrollments', id: existingId, data, overrideAccess: true });
          }
          ...
          const created = await payload.create({ collection: 'enrollments', data: data as never, overrideAccess: true });
          existingMap.set(dedupeKey, created.id as number);
          record({ row: rowNumber, status: 'created', label, studentId, classId: cls.id });
        }
      } catch (err) {
        record({ row: rowNumber, status: 'error', message: `${label}: ${err instanceof Error ? err.message : 'Lỗi không xác định khi ghi DB.'}` });
      }
    }
  }
