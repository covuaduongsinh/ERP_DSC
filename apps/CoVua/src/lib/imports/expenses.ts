...
export async function importExpenses(
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (seenInFile.has(fileKey)) {
      record({ row: rowNumber, status: 'skipped', label, message: 'Trùng dòng trước trong cùng nguồn — bỏ qua.' });
      continue;
    }
    ...
    try {
      ...
      if (exists && existingId != null) {
        if (!dryRun) {
          await payload.update({ collection: 'expenses', id: existingId, data: writeData, overrideAccess: true });
        }
