 ...
 *
 * Client Keycloak: SSO_CLIENT_ID (mặc định "covua"), confidential, có SSO_CLIENT_SECRET.
 */
...
function clientId(): string {
  return process.env.SSO_CLIENT_ID || 'covua';
}
...
export async function exchangeCodeForTokens(opts: {
  ...
  const res = await fetch(
    `${serverBase()}/realms/${realm()}/protocol/openid-connect/token`,
    {
      method: 'POST',
...
export async function fetchUserInfo(accessToken: string): Promise<KeycloakUserInfo> {
  const res = await fetch(
    `${serverBase()}/realms/${realm()}/protocol/openid-connect/userinfo`,
 ...
 * Nhận cả realm role lẫn client role; chấp nhận tiền tố "covua-" (vd "covua-manager").
...
export function mapKeycloakRolesToPayload(info: KeycloakUserInfo): UserRole[] {
  ...
  const mapped = USER_ROLES.filter((role) => raw.has(role) || raw.has(`covua-${role}`));
