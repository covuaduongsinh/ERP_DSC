...
const COVUA_STREAM = 'VIERP_COVUA';
const COVUA_SUBJECTS = ['covua.>'];
...
async function getJetStream(): Promise<JetStreamClient | null> {
  ...
  nc = await connect({
    servers: url,
    name: `erp-${process.env.MODULE_NAME || 'covua'}`,
    reconnect: true,
    maxReconnectAttempts: -1,
    reconnectTimeWait: 2000,
  });

  // Đảm bảo stream covua.> tồn tại (idempotent).
  const jsm = await nc.jetstreamManager();
  try {
    await jsm.streams.info(COVUA_STREAM);
  } catch {
    await jsm.streams.add({
      name: COVUA_STREAM,
      subjects: COVUA_SUBJECTS,
    ...
    console.log(`[covua-events] Created JetStream stream ${COVUA_STREAM}`);
  }
...
export type CovuaEventEnvelope<T> = {
  id: string;
  type: string;
  source: string;
  timestamp: string;
...
/**
 * Phát một sự kiện CoVua lên NATS JetStream (subject dạng `covua.*`).
 * Envelope khớp định dạng của @vierp/events để subscriber ERP đọc thống nhất.
 */
export async function publishCovuaEvent<T>(
  subject: string,
  data: T,
  ...
  try {
    ...
    const envelope: CovuaEventEnvelope<T> = {
    ...
    console.error('[covua-events] publish failed:', (err as Error).message);
  }
}
