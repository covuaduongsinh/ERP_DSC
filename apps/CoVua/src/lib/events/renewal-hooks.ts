 ...
 * `covua.renewal.requested` lên NATS để CRM kích hoạt chăm sóc giữ chân
 * (tạo task/lịch follow-up cho lễ tân + campaign nurture).
 *
 * Chỉ phát trên `create`. Lỗi publish được nuốt trong publishCovuaEvent ⇒ không
...
export const publishRenewalRequested: CollectionAfterChangeHook = async ({
  ...
  await publishCovuaEvent('covua.renewal.requested', data, { userId })
  return doc
}
