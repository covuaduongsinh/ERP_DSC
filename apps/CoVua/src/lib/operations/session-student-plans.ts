  ...
  const { docs: enrollments } = await payload.find({
    ...
    where: {
      and: [
        ...
        { or: [{ ngayBatDau: { exists: false } }, { ngayBatDau: { less_than_equal: dayEnd.toISOString() } }] },
        {
          or: [
            { dangHoc: { equals: true } },
...
export async function loadSessionStudentPlans(
  ...
  try {
    ...
    if (!session) {
      return { ok: false, error: 'forbidden', message: 'Không tìm thấy buổi (hoặc ngoài phạm vi cơ sở).' };
    }
...
export async function updateStudentPlan(
  ...
  if (
    !Number.isInteger(input.sessionId) || input.sessionId <= 0 ||
    !Number.isInteger(input.studentId) || input.studentId <= 0
  ) {
  ...
  try {
    ...
    const classId = relId(session.lop);
    if (classId === null) {
      return { ok: false, error: 'invalid_input', message: 'Buổi không gắn lớp — không lập kế hoạch theo HV được.' };
    }
    const roster = await loadClassRosterForDate(payload, actor, classId, session.date as string);
    if (!roster.some((r) => r.studentId === input.studentId)) {
      return { ok: false, error: 'forbidden', message: 'Học viên không thuộc lớp buổi này (hoặc ngoài cơ sở của bạn).' };
    }
    ...
    return { ok: false, error: 'server', message: 'Không lưu được kế hoạch học viên (có thể ngoài phạm vi cơ sở).' };
  }
}
