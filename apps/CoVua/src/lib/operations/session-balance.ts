...
export function computeBalance(
  input: SessionBalanceComputeInput,
): SessionBalanceComputeResult {
  const lifetimePaid = input.payments.reduce(
    (s, p) => s + normSessions(p.soBuoiNop),
    0,
  );
  const lifetimeAttended = input.attendanceDates.reduce(
    (n, d) => (utcDayKey(d) ? n + 1 : n),
    0,
  );
