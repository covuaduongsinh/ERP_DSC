...
function toSafeSlots(lichHoc: Class['lichHoc']): OpenClassSlot[] {
  ...
  return slots.sort(
    (a, b) => DAY_ORDER.indexOf(a.thu) - DAY_ORDER.indexOf(b.thu),
  );
}
...
function minLevelIndex(levels: OpenClassCard['level']): number {
  const idxs = (levels ?? [])
    .map((level) => levelOrder.indexOf(level))
    .filter((i) => i >= 0);
...
export function groupOpenClassesByLevel(
  cards: OpenClassCard[],
): OpenClassLevelGroup[] {
  return levelOrder
    .map((level) => ({
 ...
 */
export function groupOpenClassesByTrack(
  cards: OpenClassCard[],
): OpenClassTrackGroup[] {
  return trackOrder
    .map((track) => ({
