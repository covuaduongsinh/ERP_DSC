...
function TextBlock({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="mb-3">
      ...
      <p className="whitespace-pre-line text-sm leading-relaxed text-[var(--ds-text)]">
        {value}
      </p>
    </div>
  );
    ...
    <ol className="space-y-4">
      {reports.map((report) => {
        ...
        return (
          <li
            key={report.id}
            className="rounded-xl border border-primary/10 bg-[var(--ds-bg)] p-4"
          >
