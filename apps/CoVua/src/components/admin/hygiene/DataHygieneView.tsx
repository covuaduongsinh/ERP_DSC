        ...
        <header className="ds-pghead" style={{ marginBottom: 18 }}>
          <div>
            ...
            <p className="ds-pghead__sub">
              Các bản ghi còn thiếu thông tin quan trọng — dọn dần để báo cáo &
              phân quyền chính xác. Hệ thống không khóa cứng, chỉ nhắc.
            </p>
          </div>
        </header>

        {total === 0 ? (
          <p className="ds-success">Không có dữ liệu nào cần hoàn thiện trong phạm vi của bạn. 🎉</p>
        ) : (
          <div>
            {signals.map((s) => (
              <section key={s.key} className="ds-section">
                <h2 className="ds-section__title" style={{ marginBottom: 4 }}>
                  {s.label}{' '}
                  <span className={s.count > 0 ? 'ds-count--warn' : 'ds-count--ok'}>({s.count})</span>
                </h2>
                ...
                {s.count === 0 ? (
                  ...
                  <ul className="ds-samplelist">
                    ...
                    {s.count > s.samples.length ? (
                      <li className="ds-samplelist__more">… và {s.count - s.samples.length} bản ghi khác</li>
                    ) : null}
                  </ul>
                )}
              </section>
