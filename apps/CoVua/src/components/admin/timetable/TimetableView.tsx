      ...
      <Gutter>
        <header style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: 'var(--theme-elevation-500)' }}>
            Lịch các lớp theo cơ sở. Đặt lịch ở từng lớp (mục <strong>Lịch học</strong>).
            Cảnh báo bên dưới nếu một giáo viên hoặc phòng bị xếp trùng giờ.
          </p>
        </header>
        ...
        {conflicts.length > 0 ? (
          <div role="alert" className="ds-error" style={{ marginBottom: 16 }}>
            ...
            <ul style={{ margin: '6px 0 0', paddingLeft: 18 }}>
              {conflicts.map((c, i) => (
                <li key={i}>
                  {c.kind === 'coach' ? 'Trùng GV' : 'Trùng phòng'} ({DAY_LABEL[c.thu as DayOfWeek] ?? c.thu}):{' '}
                  <strong>{c.a.title}</strong> × <strong>{c.b.title}</strong> — {c.detail}
                </li>
              ))}
            </ul>
          </div>
        ...
        {totalSlots === 0 ? (
          <p className="ds-muted">
            Chưa có lớp nào đặt lịch (hoặc ngoài phạm vi cơ sở của bạn). Mở một lớp →
            điền mục <strong>Lịch học</strong> để hiện ở đây.
          </p>
        ) : (
            ...
            {DAY_ORDER.map((day) => {
              ...
              return (
                <div key={day} style={{ flex: '1 0 140px', minWidth: 140 }}>
                  <div className="ds-muted" style={{ fontWeight: 600, marginBottom: 6, textAlign: 'center' }}>
                    {DAY_LABEL[day]}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {list.length === 0 ? (
                      <div className="ds-muted" style={{ textAlign: 'center', fontSize: 12, opacity: 0.6 }}>
                        —
                      </div>
                    ) : (
                      list.map((chip, i) => (
