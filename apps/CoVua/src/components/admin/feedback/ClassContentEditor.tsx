...
export function ClassContentEditor({
  ...
  return (
    <fieldset style={{ border: '1px solid var(--theme-elevation-150)', borderRadius: 6, padding: 12 }}>
      ...
      {error && <p role="alert" className="ds-error" style={{ margin: '0 0 8px' }}>{error}</p>}
      {saved && <p aria-live="polite" className="ds-success" style={{ margin: '0 0 8px' }}>Đã lưu nội dung buổi.</p>}

      {enableTemplateFill && <TemplateFillPicker onFill={fillFromLesson} disabled={saving} />}

      <label className="ds-field" style={{ display: 'block' }}>
        Mục tiêu
        <textarea className="ds-input ds-input--full" rows={2} value={form.mucTieu} onChange={(e) => set('mucTieu', e.target.value)} disabled={saving} />
      </label>
      ...
      <label className="ds-field" style={{ display: 'block', marginTop: 8 }}>
        Giao BTVN
        <textarea className="ds-input ds-input--full" rows={2} value={form.giaoBTVN} onChange={(e) => set('giaoBTVN', e.target.value)} disabled={saving} />
      </label>
      <label className="ds-field" style={{ display: 'block', marginTop: 8 }}>
        Kế hoạch buổi sau
...
function TemplateFillPicker({
  ...
  if (!open) {
    return (
      <div style={{ marginBottom: 10 }}>
        <Button buttonStyle="secondary" type="button" disabled={disabled} onClick={() => setOpen(true)}>
  ...
  return (
    <div style={{ border: '1px dashed var(--theme-elevation-200)', borderRadius: 6, padding: 10, marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <strong>Điền từ khung lộ trình</strong>
        <button type="button" className="ds-tbl__link" onClick={() => setOpen(false)}>Đóng</button>
      </div>
      {templates && templates.length === 0 ? (
        ...
        <>
          <div className="ds-formrow" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <label className="ds-field" style={{ flex: '1 1 200px' }}>
              Khung
              <select className="ds-select" value={templateId} onChange={(e) => setTemplateId(Number(e.target.value))} disabled={disabled || busy}>
                {(templates ?? []).map((t) => (
            ...
            <label className="ds-field" style={{ flex: '1 1 220px' }}>
              Bài
              <select className="ds-select" value={lessonIdx} onChange={(e) => setLessonIdx(Number(e.target.value))} disabled={disabled || busy || !lessons?.length}>
                {(lessons ?? []).map((l, i) => (
                  <option key={i} value={i}>
                    Bài {i + 1}{l.tieuDe ? ` · ${l.tieuDe}` : ''}
                  </option>
                ))}
              </select>
            </label>
            <Button buttonStyle="secondary" type="button" disabled={disabled || busy || !lessons?.length} onClick={onFillClick}>
              Điền vào ô bên dưới
            </Button>
          </div>
          <p className="ds-muted" style={{ margin: '6px 0 0' }}>
            Điền <strong>Mục tiêu · Sách · Kiến thức mới · Giao BTVN</strong> từ bài đã chọn (ô bài trống thì giữ nguyên). “Kế hoạch buổi sau” không bị đụng.
          </p>
        </>
      )}
      {msg && <p aria-live="polite" className="ds-success" style={{ margin: '6px 0 0' }}>{msg}</p>}
    </div>
  );
}
