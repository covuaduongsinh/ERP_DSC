...
export const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    icon: Megaphone,
    items: [
      { id: 'leads', label: 'Lead tư vấn', href: '/admin/lead-pipeline', icon: UserPlus, badge: 'leads' },
    ],
  },
  {
    group: 'Đào tạo',
    ...
    items: [
      ...
      { id: 'enrollments', label: 'Ghi danh', href: '/admin/collections/enrollments', icon: FilePlus2 },
      { id: 'roster', label: 'Sĩ số lớp', href: '/admin/si-so-lop', icon: ClipboardList },
      { id: 'studentEnrollment', label: 'Ghi danh HV', href: '/admin/ghi-danh-hoc-vien', icon: FilePlus2 },
      { id: 'timetable', label: 'Thời khóa biểu', href: '/admin/thoi-khoa-bieu', icon: CalendarClock },
      { id: 'levels', label: 'Cấp độ', href: '/admin/collections/levels', icon: Layers },
      { id: 'attendance', label: 'Điểm danh', href: '/admin/diem-danh-nhanh', icon: ClipboardCheck },
      { id: 'sessionEntry', label: 'Buổi học', href: '/admin/buoi-hoc', icon: NotebookPen },
      { id: 'sessionPlanning', label: 'Lập kế hoạch buổi', href: '/admin/lap-ke-hoach-lop', icon: CalendarDays },
      { id: 'holidays', label: 'Lịch nghỉ', href: '/admin/collections/holidays', icon: CalendarOff },
      { id: 'curriculumTemplates', label: 'Soạn khung lộ trình', href: '/admin/soan-khung-lo-trinh', icon: BookOpen },
      { id: 'sessionFeedback', label: 'Nhận xét buổi', href: '/admin/nhan-xet-buoi', icon: MessageSquareText },
      { id: 'teachingStats', label: 'Thống kê dạy', href: '/admin/thong-ke-day', icon: BarChart3 },
      { id: 'reports', label: 'Báo cáo tiến độ', href: '/admin/phat-hanh-bao-cao', icon: FileBarChart, badge: 'reports' },
    ],
  },
  {
    group: 'Tài chính',
    ...
    items: [
      ...
      { id: 'tuitionPackages', label: 'Gói học phí', href: '/admin/collections/tuition-packages', icon: Package },
      { id: 'renewals', label: 'Gia hạn', href: '/admin/duyet-gia-han', icon: RefreshCw, badge: 'renewals' },
      ...
      { id: 'expenses', label: 'Phiếu chi', href: '/admin/collections/expenses', icon: TrendingDown },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'books', label: 'Sách nổi bật', href: '/admin/collections/featured-books', icon: BookOpen },
      { id: 'webLocations', label: 'Lịch học cơ sở', href: '/admin/collections/web-locations', icon: MapPin },
      { id: 'webCoaches', label: 'Huấn luyện viên (web)', href: '/admin/collections/web-coaches', icon: GraduationCap },
      { id: 'webPrograms', label: 'Lớp đang tuyển (cũ)', href: '/admin/collections/web-programs', icon: Layers },
    ],
  },
  {
    group: 'Hệ thống',
    icon: Settings,
    items: [
      { id: 'coaches', label: 'Giáo viên', href: '/admin/collections/coaches', icon: GraduationCap },
      { id: 'locations', label: 'Cơ sở', href: '/admin/collections/locations', icon: MapPin },
      { id: 'dataHygiene', label: 'Dữ liệu cần hoàn thiện', href: '/admin/du-lieu-can-hoan-thien', icon: AlertTriangle },
      ...
      { id: 'audit', label: 'Nhật ký kiểm toán', href: '/admin/collections/audit-logs', icon: ScrollText },
    ],
  },
];
...
export const ROLE_NAV: Record<UserRole, NavItemId[]> = {
  ...
  accountant: ['dashboard', 'students', 'classes', 'payments', 'tuitionPackages', 'renewals', 'debt', 'expenses', 'refunds', 'payroll', 'reports', 'roster', 'sessionBalance'],
  receptionist: ['dashboard', 'leads', 'students', 'classes', 'attendance', 'sessionEntry', 'sessionPlanning', 'sessionFeedback', 'reports', 'posts', 'events', 'books', 'webLocations', 'webCoaches', 'webPrograms', 'roster', 'enrollments', 'studentEnrollment', 'timetable', 'tuitionPackages'],
  coach: ['dashboard', 'students', 'classes', 'attendance', 'sessionEntry', 'sessionPlanning', 'sessionFeedback', 'teachingStats', 'reports', 'roster', 'timetable'],
  assistant: ['dashboard', 'classes', 'attendance', 'sessionEntry', 'sessionPlanning', 'sessionFeedback', 'teachingStats', 'students', 'roster', 'timetable'],
};
...
export const QUICK_ACTIONS: QuickAction[] = [
  { href: '/admin/diem-danh-nhanh', label: 'Điểm danh nhanh', icon: ClipboardCheck, requires: 'attendance' },
