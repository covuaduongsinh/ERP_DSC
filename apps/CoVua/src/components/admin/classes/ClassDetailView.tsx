...
async function buildData(
  ...
  for (const p of payRes?.docs ?? []) {
    ...
    arr.push({ ngayNop: (p as { ngayNop?: unknown }).ngayNop, soBuoiNop: (p as { soBuoiNop?: unknown }).soBuoiNop });
  ...
  const attendanceAvg = withAtt.length
    ? Math.round(
        (withAtt.reduce((sum, s) => sum + (s.present ?? 0) / (s.total as number), 0) / withAtt.length) * 100,
      )
    : null;
  ...
  const days = Array.from(new Set(lichHoc.map((l) => (l.thu ? THU_LABEL[l.thu] ?? l.thu : '')).filter(Boolean)));
    ...
    body = data ? (
      ...
      <>
        <p style={{ marginBottom: 12 }}>
          <Link href="/admin/collections/classes" style={{ color: 'var(--theme-text)', textDecoration: 'underline' }}>
            ← Danh sách lớp
          </Link>
        </p>
