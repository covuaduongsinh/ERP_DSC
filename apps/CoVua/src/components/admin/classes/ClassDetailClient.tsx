...
function initials(name: string): string {
  const parts = name.replace(/^(Chị|Anh|Bé|Em)\s+/i, '').trim().split(/\s+/);
...
function Piece({ piece, size = 22 }: { piece: string; size?: number }) {
  return (
    ...
    <img src={`/brand/pieces/${piece}.svg`} alt="" className="ds-piece" style={{ width: size, height: size }} />
  );
}
...
function SessionRow({
  ...
  return (
    <>
      <tr
        className={'ds-tbl__rowclick' + (planned ? ' ds-tbl__rowmuted' : '') + (open ? ' is-open' : '')}
        onClick={onToggle}
        aria-expanded={open}
      >
        <td>
        ...
        <td>
          {s.total ? (
            <b className="ds-tnum" style={{ color: s.present === s.total ? 'var(--ds-green)' : undefined }}>
              {s.present}/{s.total}
            </b>
          ) : (
          ...
          <td colSpan={5}>
            <div className="ds-sessdetail">
              ...
              <div className="ds-sessactions">
                <Link href={`/admin/nhan-xet-buoi?session=${s.id}&${from}`} className="ds-btn ds-btn--ghost ds-btn--sm">
                ...
                <Link href={`/admin/diem-danh-nhanh?${from}`} className="ds-btn ds-btn--ghost ds-btn--sm">
    ...
    <div className="ds-page">
      ...
      <div className="ds-card ds-clshead">
        ...
        <div className="ds-clshead__actions">
          <Link href={`/admin/lap-ke-hoach-lop?${fromFor('sessions')}`} className="ds-btn ds-btn--ghost">
            <CalendarDays aria-hidden />
            Lập kế hoạch
          </Link>
          <Link href={`/admin/diem-danh-nhanh?${fromFor('sessions')}`} className="ds-btn ds-btn--primary">
            <ClipboardCheck aria-hidden />
            Điểm danh
          </Link>
        </div>
      ...
      <div className="ds-tabs">
        <button type="button" className={tab === 'students' ? 'is-on' : ''} onClick={() => setTab('students')}>
          Học viên ({data.roster.length})
        </button>
        <button type="button" className={tab === 'sessions' ? 'is-on' : ''} onClick={() => setTab('sessions')}>
          Buổi học
        </button>
        <button type="button" className={tab === 'curriculum' ? 'is-on' : ''} onClick={() => setTab('curriculum')}>
          Khung lộ trình
        </button>
      </div>
              ...
              <tbody>
                ...
                {data.roster.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>{s.level ? <LevelPill level={s.level} /> : <span className="ds-muted">—</span>}</td>
                    <td>
      ...
      {tab === 'curriculum' && (
        <div className="ds-panel">
          <div className="ds-panel__head">
            ...
            <h2 className="ds-panel__t">{data.curriculum ? data.curriculum.title : 'Chưa gán khung lộ trình'}</h2>
            {data.curriculum && (
              <Link href={`/admin/soan-khung-lo-trinh?${fromFor('curriculum')}`} className="ds-btn ds-btn--ghost ds-btn--sm">
                Soạn khung
              </Link>
            )}
          </div>
              ...
              {lessons.map((t, i) => {
                ...
                return (
                  <div key={i} className={`ds-currlesson ds-currlesson--${state}`}>
                    ...
                    {state === 'next' && <span className="ds-badge ds-badge--info ds-ml-auto">Buổi tới</span>}
                  </div>
                );
              })}
              {lessons.length === 0 && (
