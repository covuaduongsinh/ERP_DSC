...
export function PaymentEntryClient({ students }: PaymentEntryClientProps) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      ...
      const total =
        (parseNum(hocPhi) ?? 0) + (parseNum(tienSach) ?? 0) + (parseNum(muaKhac) ?? 0);
      setRecent((prev) => [
        {
    ...
    <div>
      ...
      <div className="ds-card" style={{ maxWidth: 720 }}>
        ...
        <div className="ds-formrow">
          <label className="ds-field">
            Học phí (đ)
            <input className="ds-input" inputMode="numeric" value={hocPhi} onChange={(e) => setHocPhi(e.target.value)} disabled={isPending} />
          </label>
          <label className="ds-field">
            Tiền sách (đ)
            <input className="ds-input" inputMode="numeric" value={tienSach} onChange={(e) => setTienSach(e.target.value)} disabled={isPending} />
          </label>
          <label className="ds-field">
            Mua khác (đ)
            <input className="ds-input" inputMode="numeric" value={muaKhac} onChange={(e) => setMuaKhac(e.target.value)} disabled={isPending} />
          </label>
          <label className="ds-field">
            Số buổi nộp
            <input className="ds-input" inputMode="numeric" value={soBuoiNop} onChange={(e) => setSoBuoiNop(e.target.value)} disabled={isPending} />
          </label>
        </div>

        <label className="ds-field ds-field--wide" style={{ marginBottom: 12 }}>
          Ghi chú (tùy chọn)
          <textarea className="ds-input ds-input--full" value={ghiChu} onChange={(e) => setGhiChu(e.target.value)} disabled={isPending} rows={2} />
        </label>

        <Button buttonStyle="primary" type="button" disabled={isPending || studentId === ''} onClick={submit}>
