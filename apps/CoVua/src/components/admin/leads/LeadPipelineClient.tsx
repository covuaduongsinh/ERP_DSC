...
import { Phone, MessageCircle, ArrowRightLeft, X, CircleSlash, Clock, CheckCircle2 } from 'lucide-react';
...
function initials(name: string): string {
  ...
  const t = parts.length >= 2 ? parts[parts.length - 2][0] + parts[parts.length - 1][0] : name.slice(0, 2);
...
export function LeadPipelineClient({ initialCards }: { initialCards: LeadCard[] }) {
  ...
  const convert = (id: number) => {
    startTransition(async () => {
      ...
      if (res.ok) {
        setCards((prev) => prev.map((c) => (c.id === id ? { ...c, status: 'da_chot', converted: true } : c)));
        showToast(res.alreadyConverted ? 'Lead đã được chuyển đổi trước đó' : 'Đã chuyển đổi → Phụ huynh + Học viên');
        router.refresh();
      } else {
                ...
                {colCards.map((c) => (
                  ...
                  >
                    ...
                    <div className="ds-lead__meta">
                      ...
                      {c.converted && <span className="ds-badge ds-badge--success">Đã chuyển đổi</span>}
                    </div>
        ...
        <>
          ...
          <aside className="ds-drawer" role="dialog" aria-label={`Lead ${active.fullName}`}>
            <div className="ds-drawer__head">
              <span
                className="ds-avatar"
                style={{ background: 'linear-gradient(140deg, var(--ds-teal), var(--ds-blue-light))' }}
                aria-hidden
              >
              ...
              <button type="button" className="ds-iconbtn" onClick={() => setActiveId(null)} aria-label="Đóng">
                <X aria-hidden />
              </button>
            </div>
              ...
              <div className="ds-drawer__sec">
                ...
                <dl className="ds-kvlist">
                  ...
                  {active.nextFollowUpAt && (
                    <div>
                      <dt>Hẹn liên hệ</dt>
                      <dd className={active.overdue ? 'is-overdue' : ''}>{fmtDateTime(active.nextFollowUpAt)}</dd>
                    </div>
                  )}
                  {active.note && (
                    <div>
                  ...
                  <ul className="ds-timeline">
                    {active.activity.map((a, i) => (
                      <li key={i} className="ds-timeline__row">
                        <span className="ds-timeline__type">{ACTIVITY_LABELS[a.type] ?? a.type}</span>
