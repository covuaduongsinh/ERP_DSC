...
type Form = { mucTieu: string; sachDangHoc: string; kienThucMoi: string; giaoBTVN: string; khBuoiSau: string };
...
function inheritPlaceholder(classContent: StudentPlanContent | null, key: keyof StudentPlanContent): string {
...
export function StudentPlanTable({ sessionId }: { sessionId: number }) {
    ...
    startBusy(async () => {
      ...
      setMsg(
        fail === 0
          ...
          : { kind: 'err', text: `Đã lưu ${ok} học viên; ${fail} dòng lỗi (có thể ngoài phạm vi cơ sở).` },
      );
    });
  };
  ...
  if (rows === null) return <p className="ds-muted" style={{ margin: '8px 0' }}>Đang tải học viên…</p>;
  ...
  return (
    <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {note && rows.length === 0 ? (
        <p className="ds-muted" style={{ margin: 0 }}>{note}</p>
      ) : (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <strong>Kế hoạch theo học viên ({rows.length})</strong>
            <span style={{ display: 'inline-flex', gap: 4 }}>
              <Button buttonStyle={view === 'table' ? 'primary' : 'secondary'} type="button" disabled={busy} onClick={() => setView('table')}>
                Bảng
              </Button>
              <Button buttonStyle={view === 'card' ? 'primary' : 'secondary'} type="button" disabled={busy} onClick={() => setView('card')}>
                Thẻ
              </Button>
            </span>
            <Button buttonStyle="primary" type="button" disabled={busy || dirtyIds.length === 0} onClick={saveAllDirty}>
              {busy ? 'Đang lưu…' : `Lưu tất cả thay đổi${dirtyIds.length ? ` (${dirtyIds.length})` : ''}`}
            </Button>
          ...
          {view === 'table' ? (
            <div className="ds-tbl-wrap">
              <table className="ds-tbl">
                <thead>
                  <tr>
                    <th scope="col" style={{ minWidth: 120 }}>Học viên</th>
                    {FIELD_DEFS.map((f) => (
                      <th key={f.key} scope="col" style={{ minWidth: 180 }}>{f.label}</th>
                    ))}
                    <th scope="col" style={{ width: 90 }}>Lưu</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r) => {
                    ...
                    return (
                      <tr key={r.studentId} style={dirty ? { background: 'var(--theme-elevation-50)' } : undefined}>
                        <td style={{ fontWeight: 600, verticalAlign: 'top' }}>
                          {r.fullName}
                          {dirty && <span className="ds-muted" style={{ display: 'block', fontWeight: 400 }}>• chưa lưu</span>}
                        </td>
                        {FIELD_DEFS.map((f) => (
                        ...
                        <td style={{ verticalAlign: 'top' }}>
                          <Button buttonStyle="primary" type="button" disabled={busy || !dirty} onClick={() => saveOne(r.studentId, r.fullName)}>
                            Lưu
                          </Button>
                        </td>
                      </tr>
...
function StudentCard({
  ...
  return (
    <fieldset style={{ border: '1px solid var(--theme-elevation-150)', borderRadius: 6, padding: 10 }}>
      <legend style={{ padding: '0 6px', fontWeight: 600 }}>
        {row.fullName}{dirty ? ' • chưa lưu' : ''}
      </legend>
      <div className="ds-formrow" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <label className="ds-field" style={{ flex: '1 1 100%' }}>
          Mục tiêu
          <textarea className="ds-input ds-input--full" rows={2} value={f.mucTieu} placeholder={inheritPlaceholder(classContent, 'mucTieu')} onChange={(e) => onField('mucTieu', e.target.value)} disabled={busy} />
        </label>
      </div>
      <div className="ds-formrow" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
        <label className="ds-field" style={{ flex: '1 1 220px' }}>
          Sách đang học
          <input className="ds-input ds-input--full" type="text" value={f.sachDangHoc} placeholder={inheritPlaceholder(classContent, 'sachDangHoc')} onChange={(e) => onField('sachDangHoc', e.target.value)} disabled={busy} />
        </label>
        <label className="ds-field" style={{ flex: '1 1 220px' }}>
          Kiến thức mới
          <textarea className="ds-input ds-input--full" rows={2} value={f.kienThucMoi} placeholder={inheritPlaceholder(classContent, 'kienThucMoi')} onChange={(e) => onField('kienThucMoi', e.target.value)} disabled={busy} />
        </label>
      </div>
      <div className="ds-formrow" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
        <label className="ds-field" style={{ flex: '1 1 220px' }}>
          Giao BTVN
          <textarea className="ds-input ds-input--full" rows={2} value={f.giaoBTVN} placeholder={inheritPlaceholder(classContent, 'giaoBTVN')} onChange={(e) => onField('giaoBTVN', e.target.value)} disabled={busy} />
        </label>
        <label className="ds-field" style={{ flex: '1 1 220px' }}>
          Kế hoạch buổi sau
          <textarea className="ds-input ds-input--full" rows={2} value={f.khBuoiSau} placeholder={inheritPlaceholder(classContent, 'khBuoiSau')} onChange={(e) => onField('khBuoiSau', e.target.value)} disabled={busy} />
        </label>
      </div>
