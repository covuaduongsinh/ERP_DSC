...
const TRANG_THAI_LABEL: Record<string, string> = {
  du_kien: 'Dự kiến', da_day: 'Đã dạy', huy: 'Hủy', bu: 'Dạy bù',
};
...
export function SessionPlanningClient({
  ...
  const [preview, setPreview] = useState<{ outcomes: PlanOutcome[]; toCreate: number; existing: number; skipped: number; note: string | null } | null>(null);
  const [sessions, setSessions] = useState<PlannedSessionRow[] | null>(null);
  const [openRow, setOpenRow] = useState<{ id: number; mode: 'content' | 'students' | 'makeup' | 'cancel' } | null>(null);
  ...
  const onPreview = () => {
    ...
    startBusy(async () => {
      ...
      setPreview({ outcomes: res.outcomes, toCreate: res.toCreate, existing: res.existing, skipped: res.skipped, note: res.note });
  ...
  const onMakeup = (row: PlannedSessionRow, date: string) => {
    setMsg(null);
    startBusy(async () => {
      const res = await createPlannedMakeupAction({ classId: lopId, originalSessionId: row.sessionId, date });
      if (!res.ok) {
        setMsg({ kind: 'err', text: res.message });
        return;
      }
  ...
  const onCancelHolidayConflicts = (conflicts: PlanOutcome[]) => {
    ...
    startBusy(async () => {
      ...
      if (pv.ok) setPreview({ outcomes: pv.outcomes, toCreate: pv.toCreate, existing: pv.existing, skipped: pv.skipped, note: pv.note });
      await refreshList();
    });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <fieldset style={{ border: '1px solid var(--theme-elevation-150)', borderRadius: 6, padding: 12 }}>
        ...
        <div className="ds-formrow" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <label className="ds-field" style={{ flex: '1 1 240px' }}>
            Lớp
            <select className="ds-select" value={lopId} onChange={(e) => setLopId(Number(e.target.value))} disabled={busy}>
              {classes.map((c) => (
          ...
          <label className="ds-field">
            Từ ngày
            <input className="ds-input" type="date" value={from} onChange={(e) => setFrom(e.target.value)} disabled={busy} />
          </label>
          <label className="ds-field">
            Đến ngày
            <input className="ds-input" type="date" value={to} onChange={(e) => setTo(e.target.value)} disabled={busy} />
          </label>
          <div style={{ display: 'flex', gap: 8 }}>
            <Button buttonStyle="secondary" type="button" disabled={busy || !lopId} onClick={onPreview}>
              {busy ? 'Đang xử lý…' : 'Kiểm tra (xem trước)'}
            </Button>
            <Button buttonStyle="secondary" type="button" disabled={busy || !lopId} onClick={() => { setMsg(null); void refreshList(); }}>
              Tải danh sách buổi
            </Button>
          </div>
        </div>
      ...
      {msg && (
        <p role={msg.kind === 'err' ? 'alert' : undefined} className={msg.kind === 'err' ? 'ds-error' : 'ds-success'} style={{ margin: 0 }}>
          {msg.text}
        </p>
      )}
      ...
      {preview && preview.outcomes.length > 0 && (
        <div className="ds-tbl-wrap">
          ...
          {canPlan && holidayConflicts.length > 0 && (
            <div role="alert" className="ds-error" style={{ margin: '0 0 8px', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontWeight: 600 }}>
                Có {holidayConflicts.length} buổi dự kiến đã lỡ sinh trùng ngày nghỉ.
              </span>
              <Button buttonStyle="secondary" type="button" disabled={busy} onClick={() => onCancelHolidayConflicts(holidayConflicts)}>
            ...
            <tbody>
              {preview.outcomes.map((o) => (
                <tr key={o.date}>
                  ...
                  <td>{o.gioBatDau}–{o.gioKetThuc}</td>
                  <td>{o.phong ?? '—'}</td>
                  <td>
                    {o.status === 'create' ? (
                      ...
                      <span className="ds-badge ds-badge--skipped">Đã có ({TRANG_THAI_LABEL[o.trangThai ?? 'du_kien'] ?? o.trangThai})</span>
                    )}
                  </td>
                </tr>
              ))}
          ...
          {canPlan ? (
            <div style={{ marginTop: 12 }}>
              <Button buttonStyle="primary" type="button" disabled={busy || preview.toCreate === 0} onClick={onGenerate}>
                {busy ? 'Đang sinh…' : `Xác nhận sinh ${preview.toCreate} buổi dự kiến`}
              </Button>
            </div>
          ) : (
            <p role="alert" style={{ color: 'var(--theme-warning-600)', fontWeight: 600, marginTop: 12 }}>
              Chỉ quản lý hoặc admin mới được sinh buổi dự kiến. Bạn vẫn soạn được nội dung các buổi đã có.
            </p>
          )}
        </div>
      )}
      ...
      {sessions && (
        <div>
          ...
          {sessions.length === 0 ? (
            <p className="ds-muted">Chưa có buổi nào trong khoảng. Hãy xem trước rồi sinh buổi dự kiến.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sessions.map((s) => {
                const open = openRow?.id === s.sessionId ? openRow.mode : null;
                return (
                  <div key={s.sessionId} style={{ border: '1px solid var(--theme-elevation-150)', borderRadius: 6, padding: 10 }}>
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <span className={`ds-badge ds-badge--${s.trangThai === 'huy' ? 'skipped' : 'created'}`}>
                        {TRANG_THAI_LABEL[s.trangThai] ?? s.trangThai}
                      </span>
                      {s.buoiGocId && <span className="ds-muted">(bù cho #{s.buoiGocId})</span>}
                      <span style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
                        <button type="button" className="ds-tbl__link" onClick={() => setOpenRow(open === 'content' ? null : { id: s.sessionId, mode: 'content' })}>
                          {open === 'content' ? 'Đóng' : 'Nội dung'}
                        </button>
                        <button type="button" className="ds-tbl__link" onClick={() => setOpenRow(open === 'students' ? null : { id: s.sessionId, mode: 'students' })}>
                          {open === 'students' ? 'Đóng' : 'Học viên'}
                        </button>
                        {canPlan && s.trangThai !== 'huy' && (
                          <button type="button" className="ds-tbl__link" onClick={() => setOpenRow(open === 'cancel' ? null : { id: s.sessionId, mode: 'cancel' })}>
                            Hủy buổi
                          </button>
                        )}
                        {canPlan && (
                          <button type="button" className="ds-tbl__link" onClick={() => setOpenRow(open === 'makeup' ? null : { id: s.sessionId, mode: 'makeup' })}>
                            Tạo bù
                          </button>
                        )}
                      </span>
                    ...
                    {open === 'content' && (
                      <div style={{ marginTop: 10 }}>
                        <ClassContentEditor sessionId={s.sessionId} content={s.content} enableTemplateFill />
                      </div>
                    )}
                    {open === 'students' && <StudentPlanTable sessionId={s.sessionId} />}
                    {open === 'cancel' && <CancelForm busy={busy} onConfirm={(r) => onCancel(s, r)} onClose={() => setOpenRow(null)} />}
                    {open === 'makeup' && <MakeupForm busy={busy} onConfirm={(d) => onMakeup(s, d)} onClose={() => setOpenRow(null)} />}
                  </div>
                );
              })}
            </div>
...
function CancelForm({ busy, onConfirm, onClose }: { busy: boolean; onConfirm: (reason: string) => void; onClose: () => void }) {
  ...
  return (
    <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
      <label className="ds-field" style={{ flex: '1 1 280px' }}>
        Lý do hủy
        <input className="ds-input ds-input--full" type="text" value={reason} onChange={(e) => setReason(e.target.value)} disabled={busy} placeholder="VD: Nghỉ lễ, GV bận…" />
      </label>
      <Button buttonStyle="primary" type="button" disabled={busy} onClick={() => onConfirm(reason)}>
        {busy ? 'Đang hủy…' : 'Xác nhận hủy'}
      </Button>
      <Button buttonStyle="secondary" type="button" disabled={busy} onClick={onClose}>Đóng</Button>
    </div>
  );
}

function MakeupForm({ busy, onConfirm, onClose }: { busy: boolean; onConfirm: (date: string) => void; onClose: () => void }) {
  ...
  return (
    <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
      ...
      <Button buttonStyle="primary" type="button" disabled={busy || !date} onClick={() => onConfirm(date)}>
