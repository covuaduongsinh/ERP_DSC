...
export async function RenewalQueueView(props: AdminViewServerProps) {
  ...
  const initialRequests: RenewalRow[] = docs.map((reqDoc) => {
    ...
    return {
      id: reqDoc.id,
      studentName:
        student && typeof student === 'object' ? student.fullName : 'Học viên',
      parentName:
        parent && typeof parent === 'object' ? parent.fullName : null,
      parentPhone: parent && typeof parent === 'object' ? parent.phone : null,
      currentPackage:
        cycle && typeof cycle === 'object' ? cycle.package : null,
      ...
      <Gutter>
        <header style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: 'var(--theme-elevation-500)' }}>
            Phụ huynh gửi yêu cầu gia hạn từ cổng. Duyệt để chốt gói mới (sinh chu
            kỳ học phí) hoặc từ chối kèm lý do.
          </p>
        </header>
        {canProcess ? (
          <RenewalQueueClient
          ...
          <p
            role="alert"
            style={{ color: 'var(--theme-warning-600)', fontWeight: 600 }}
          >
            Chỉ kế toán, quản lý hoặc admin mới được duyệt/từ chối yêu cầu gia
            hạn. Bạn đang xem ở chế độ chỉ đọc ({initialRequests.length} yêu cầu
            đang chờ).
          </p>
        )}
      </Gutter>
