...
const LEAD_STAGES: { key: string; label: string; color: string }[] = [
  { key: 'moi', label: 'Mới', color: '#9aa1b4' },
  { key: 'da_lien_he', label: 'Đã liên hệ', color: 'var(--ds-blue-light)' },
  { key: 'dang_ky_hoc_thu', label: 'Học thử', color: 'var(--ds-teal)' },
  { key: 'da_chot', label: 'Đã chốt', color: 'var(--ds-green)' },
];
...
export async function AdminDashboard(props: AdminViewServerProps) {
  ...
  // Phễu: đếm lead theo từng giai đoạn (role không đọc được leads ⇒ 0 ⇒ ẩn panel).
  const funnelCounts = await Promise.all(
    LEAD_STAGES.map((s) =>
      payload
        .count({
          collection: 'leads',
          where: { status: { equals: s.key } },
          user: staff,
  ...
  const queueItems: {
    ...
    { key: 'overdue', label: 'Follow-up quá hạn', count: queue.overdueFollowUpLeads, href: '/admin/collections/leads?sort=nextFollowUpAt', hint: 'Lead đang chăm đã trễ hẹn', tone: 'critical', icon: AlarmClock },
    { key: 'uncared', label: 'Lead chưa chăm', count: queue.uncaredLeads, href: '/admin/collections/leads?where[status][equals]=moi', hint: 'Lead mới gửi, chưa ai liên hệ', tone: 'warning', icon: BellRing },
    { key: 'renewal', label: 'Gia hạn đang chờ', count: queue.pendingRenewalRequests, href: '/admin/duyet-gia-han', hint: 'Phụ huynh xin gia hạn, chờ xử lý', tone: 'warning', icon: RefreshCw },
    { key: 'low', label: 'Sắp hết buổi', count: queue.lowSessionStudents, href: '/admin/cong-no', hint: 'Học viên sắp hết số buổi tồn', tone: 'warning', icon: BatteryLow },
    { key: 'reconcile', label: 'Cần đối soát buổi', count: queue.reconcileStudents, href: '/admin/so-buoi-ton', hint: 'Tồn âm hoặc chưa có phiếu thu', tone: 'info', icon: Calculator },
    { key: 'draft', label: 'Báo cáo còn nháp', count: queue.draftProgressReports, href: draftReportsHref, hint: 'Nhận xét định kỳ chưa phát hành', tone: 'info', icon: FilePenLine },
  ];
  ...
  return (
    <Gutter className="dashboard">
      <div className="ds-dash">
          ...
          {/* Phễu pipeline + Lớp tại cơ sở */}
          <div className="ds-grid2">
            {funnelTotal > 0 && (
              <div className="ds-panel">
                <div className="ds-panel__head">
                  <Filter className="ds-panel__ic" aria-hidden />
                  <h2 className="ds-panel__t">Phễu pipeline</h2>
                </div>
                <div className="ds-panel__body">
                  <div className="ds-funnel">
                    {LEAD_STAGES.map((s, i) => (
                      <div className="ds-frow" key={s.key}>
                        <div
                          className="ds-frow__bar ds-tnum"
                          style={{ width: `${30 + (funnelCounts[i] / funnelMax) * 60}%`, background: s.color }}
                        >
                          {funnelCounts[i]}
                        </div>
                        <div className="ds-frow__lab">{s.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {classes.map((c) => {
                      ...
                      return (
                        <li key={c.id} className="ds-classitem">
                          <Link href={`/admin/collections/classes/${c.id}`} className="ds-classitem__link">
                            <b>{c.title}</b>
                            <span>
                              {locName ? `${locName} · ` : ''}
