...
function ResultBanner({ result }: { result: PreviewResult }) {
  ...
  return (
    ...
    >
      <p>
        <strong>{fileName}</strong> — {totalRows} dòng:{' '}
        <strong>{created}</strong> {dryRun ? 'sẽ tạo mới' : 'đã tạo mới'},{' '}
        <strong>{updated}</strong> {dryRun ? 'sẽ cập nhật' : 'đã cập nhật'},{' '}
        <strong>{skipped}</strong> bỏ qua (trùng),{' '}
        <strong>{errors}</strong> lỗi.
      </p>
      {dryRun ? (
        <p>
          Đây là bản XEM TRƯỚC — chưa ghi gì vào hệ thống. Kiểm tra rồi bấm “Xác
          nhận ghi”. Dòng lỗi sẽ bị bỏ qua khi ghi.
        </p>
      ) : errors === 0 ? (
...
function OutcomeTable({ result }: { result: PreviewResult }) {
  ...
  return (
    <>
      ...
      {hidden > 0 ? (
        <p className="spreadsheet-import__note">
          Đã hiển thị {shown.length}/{outcomes.length} dòng (ẩn {hidden} dòng
          tạo/cập nhật hợp lệ để gọn — mọi dòng lỗi/bỏ qua đều được hiện).
        </p>
      ) : null}
    </>
  );
