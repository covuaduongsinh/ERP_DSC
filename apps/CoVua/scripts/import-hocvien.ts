...
async function ensureNicknameColumn(payload: Awaited<ReturnType<typeof getPayload>>) {
  ...
  const exists = await pool.query('SELECT 1 FROM payload_migrations WHERE name = $1 LIMIT 1', [name]);
...
async function main() {
  ...
  const reviews = result.outcomes.filter(
    (o) => o.status === 'skipped' || o.status === 'error',
  );
  ...
  lines.push(
    `  ⚠ HV CHƯA CÓ PHỤ HUYNH  : ${result.withoutParent} (chưa onboarding được)`,
  );
