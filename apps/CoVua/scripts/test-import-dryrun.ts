...
async function main() {
  ...
  const payRows: NormalizedRow[] = [
    { ten_hoc_vien: REAL_STUDENT, ngay_nop: '2030-01-15', hoc_phi: '1.000.000', co_so: 'Kim Liên', tinh_trang: 'Đã nộp' },
  ...
  log(`  created=${payRes.created} updated=${payRes.updated} skipped=${payRes.skipped} errors=${payRes.errors} studentLinkMissing=${payRes.studentLinkMissing}`);
  ...
  log(`  created=${stuRes.created} updated=${stuRes.updated} skipped=${stuRes.skipped} errors=${stuRes.errors}`);
  for (const o of stuRes.outcomes) {
    if (o.status === 'error') log(`  [dòng ${o.row}] LỖI: ${o.message}`);
    else if (o.status === 'skipped') log(`  [dòng ${o.row}] BỎ QUA: ${o.message}`);
    else log(`  [dòng ${o.row}] ${o.status.toUpperCase()}: ${o.fullName} (hasParent=${o.hasParent})`);
  }
