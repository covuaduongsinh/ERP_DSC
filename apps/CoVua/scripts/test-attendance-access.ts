...
async function main() {
  ...
  const [pa, pb] = await Promise.all([
    payload.find({ collection: 'parents', where: { phone: { equals: '0900000001' } }, limit: 1, overrideAccess: true, depth: 0 }),
    payload.find({ collection: 'parents', where: { phone: { equals: '0900000002' } }, limit: 1, overrideAccess: true, depth: 0 }),
  ]);
  ...
  const [recA, recB] = await Promise.all([
    payload.find({ collection: 'attendance', where: { student: { equals: childA } }, limit: 1, overrideAccess: true, depth: 0 }),
    payload.find({ collection: 'attendance', where: { student: { equals: childB } }, limit: 1, overrideAccess: true, depth: 0 }),
  ]);
