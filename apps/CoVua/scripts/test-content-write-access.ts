...
async function main() {
  ...
  await payload.delete({ collection: 'locations', where: { name: { equals: locData.name } }, overrideAccess: true }).catch(() => {});
  ...
  const mkUserInvalid = (user: any) => () =>
    payload.create({ collection: 'users', data: { role: 'receptionist' } as any, overrideAccess: false, user });
  ...
  add('B3 manager: create Users KHÔNG bị access chặn (dừng ở validation)',
    b3.outcome === 'access-passed', b3.detail);

  // read = staffOnly: phụ huynh bị chặn (403, trước DB); nhân viên qua access.
  const b4 = await runOp(() => payload.find({ collection: 'users', overrideAccess: false, user: parent, limit: 1 }));
  add('B4 users.read: phụ huynh KHÔNG đọc danh sách nhân viên', b4.outcome === 'blocked', b4.detail);

  const b5 = await runOp(() => payload.find({ collection: 'users', overrideAccess: false, user: staff('coach'), limit: 1 }));
  add('B5 users.read: nhân viên (coach) qua được access', b5.outcome !== 'blocked', b5.detail);
  if (b5.outcome === 'access-passed') {
    notes.push('B5: read users qua access nhưng DB cục bộ thiếu bảng `users_sessions` nên find không hoàn tất — đây là vấn đề schema môi trường, KHÔNG phải access.');
  }

  // ── C. Leads — create=anyone (form web); read=hasRole(admin,manager,receptionist) ──
  await payload.delete({ collection: 'leads', where: { phone: { equals: '0999999999' } }, overrideAccess: true }).catch(() => {});
  const c1 = await runOp(() =>
    payload.create({ collection: 'leads', data: { fullName: 'ACCESS_TEST_LEAD', phone: '0999999999', source: 'khac', status: 'moi' }, overrideAccess: false, user: anon }));
  add('C1 leads: ẩn danh (form web) create ĐƯỢC', c1.outcome === 'ok', c1.detail);
  await payload.delete({ collection: 'leads', where: { phone: { equals: '0999999999' } }, overrideAccess: true }).catch(() => {});

  const c2 = await runOp(() => payload.find({ collection: 'leads', overrideAccess: false, user: parent, limit: 1 }));
  add('C2 leads.read: phụ huynh KHÔNG đọc được leads', c2.outcome === 'blocked', c2.detail);

  const c3 = await runOp(() => payload.find({ collection: 'leads', overrideAccess: false, user: staff('receptionist'), limit: 1 }));
