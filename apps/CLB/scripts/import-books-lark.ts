...
async function main() {
  ...
  const issueRows: BookIssueRow[] = phatsach.map((r) => {
    ...
    if (!hsName) noHs += 1
    const bookCode =
      r.sach_dung && r.sach_dung.length ? (bookCodeByRec.get(r.sach_dung[0]) ?? '') : ''
    return {
      studentName: hsName,
      bookCode,
  ...
  lines.push(
    `  Tổng: ${bRes.totalRows} | ${commit ? 'tạo' : 'sẽ tạo'}: ${bRes.created} | cập nhật: ${bRes.updated} | bỏ qua: ${bRes.skipped} | lỗi: ${bRes.errors}`,
  )

  lines.push(`\n=== Import PHÁT SÁCH (book-issues) [${commit ? 'GHI THẬT' : 'DRY-RUN'}] ===`)
  lines.push(`  Tổng: ${iRes.totalRows}`)
  lines.push(
    `  ✅ ${commit ? 'tạo' : 'sẽ tạo'}: ${iRes.created} | cập nhật: ${iRes.updated} | bỏ qua(trùng): ${iRes.skipped} | lỗi: ${iRes.errors}`,
  )
  lines.push(
    `     ↳ không khớp HV: ${iRes.studentLinkMissing} | sách không khớp code: ${iRes.bookLinkMissing}`,
  )
  ...
  if (bookErrs.length) {
    ...
    for (const o of bookErrs) {
      if (o.status === 'error' || o.status === 'skipped')
        lines.push(`  [dòng ${o.row}] ${o.message}`)
    }
  }
