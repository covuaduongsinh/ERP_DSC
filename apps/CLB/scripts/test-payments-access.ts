...
async function main() {
  ...
  await payload.update({
    collection: 'students',
    id: studentA,
    data: { parents: [parentAId] },
    overrideAccess: true,
  })
  await payload.update({
    collection: 'students',
    id: studentB,
    data: { parents: [parentBId] },
    overrideAccess: true,
  })
  ...
  const parentA = await payload.findByID({
    collection: 'parents',
    id: parentAId,
    overrideAccess: true,
    depth: 0,
  ...
  try {
    ...
    anonBlocked =
      (e as { status?: number }).status === 403 || /forbidden/i.test((e as Error).message)
  ...
  if (await serverUp()) {
    ...
    skips.push(
      'REST checks (4): dev server :3005 không chạy — bỏ qua. Local API đã phủ cùng access function.',
    )
  }
  ...
  try {
    ...
    await payload
      .delete({ collection: 'tuition-cycles', id: cycle.id, overrideAccess: true })
      .catch(() => {})
  }
