...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      r4.retryAfterMs > 0 && r4.retryAfterMs <= windowMs,
      `retryAfterMs hợp lệ (${r4.retryAfterMs}ms ~ ${Math.round(r4.retryAfterMs / 60000)} phút)`,
    )

    console.error('— Cửa sổ KHÔNG bị nới khi bị chặn —')
    assert(
      r4.retryAfterMs <= r1.retryAfterMs,
      'retryAfter của lần 4 ≤ lần 1 (expires_at giữ nguyên)',
    )
