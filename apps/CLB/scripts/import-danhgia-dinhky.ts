...
import { importProgressReports } from '../src/lib/imports/progress-reports'
...
async function main() {
  ...
  const gvEmpty = result.outcomes.filter(
    (o) => (o.status === 'created' || o.status === 'updated') && o.coachProvided === false,
  )
  const studentMiss = result.outcomes.filter(
    (o) => o.status === 'error' && o.field === 'ten_hoc_vien',
  )
  const periodErr = result.outcomes.filter((o) => o.status === 'error' && o.field === 'ky')
