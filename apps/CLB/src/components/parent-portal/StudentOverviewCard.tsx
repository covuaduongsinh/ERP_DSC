...
import { getEnrollmentStatusLabel, getTuitionStatusLabel } from '@/lib/parent-labels'
import { getTuitionWarningMessage, isTuitionLow, sessionsRemaining } from '@/lib/parent-portal'
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="truncate text-lg font-bold text-primary">{student.fullName}</h2>
          {levelLabel ? (
            <p className="mt-0.5 text-sm text-[var(--ds-text-muted)]">
              Cấp <span className="font-semibold text-primary">{levelLabel}</span>
            </p>
          ) : null}
        </div>
        <span
        ...
        {tuition ? (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <dd className="font-medium text-[var(--ds-text)]">
              Còn{' '}
              <span className={tuition.status === 'sap_het' ? 'text-primary font-bold' : ''}>
                {remaining}
              </span>{' '}
      ...
      <p className="mt-3 text-xs font-semibold text-primary">Xem tiến độ chi tiết →</p>
    </Link>
  )
}
