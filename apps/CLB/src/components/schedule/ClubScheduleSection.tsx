 ...
 */
export async function ClubScheduleSection({ variant = 'page' }: ClubScheduleSectionProps) {
  ...
  return (
    <section className="bg-bg py-16 sm:py-20">
      <div className="mx-auto max-w-container px-[18px] sm:px-7">
        <SectionHeading title={t('locationsTitle')} subtitle={t('locationsSubtitle')} />

        <Reveal className="grid gap-6 sm:grid-cols-2">
          {locations.map((loc) => {
            const times = (loc.lichHocGio ?? []).map((slot) => slot.gio).filter(Boolean)
            return (
              <article
                key={loc.id}
              ...
              >
                ...
                {(loc.lichHocNgay || times.length > 0) && (
                  <div className="mt-3 flex items-start gap-2.5 text-sm text-[#4a4f63]">
                    ...
                    <div>
                      {loc.lichHocNgay && (
                        <span className="font-semibold text-ink">{loc.lichHocNgay}</span>
                      )}
                      {times.length > 0 && (
