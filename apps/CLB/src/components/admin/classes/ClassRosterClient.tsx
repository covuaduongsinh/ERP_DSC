...
import type { ClassOption, ClassRosterRow, EnrollableStudent } from '@/lib/operations/roster'
...
export function ClassRosterClient({
  ...
  const onAdd = useCallback(
    (studentId: number) => {
      ...
      startMutate(async () => {
        ...
        setNotice(
          res.action === 'already' ? 'Học viên đã ở trong lớp.' : 'Đã thêm học viên vào lớp.',
        )
        setSearch('')
  ...
  const onBulkRemove = useCallback(() => {
    ...
    if (!window.confirm(`Rút ${ids.length} học viên đã chọn khỏi lớp? (giữ lịch sử ghi danh)`))
      return
    setError(null)
    setNotice(null)
    startMutate(async () => {
  ...
  const onBulkTransfer = useCallback(
    (toClassId: number) => {
      ...
      const target = classes.find((c) => c.id === toClassId)
      if (
        !window.confirm(`Chuyển ${ids.length} học viên đã chọn sang lớp "${target?.title ?? ''}"?`)
      )
        return
      setError(null)
      setNotice(null)
