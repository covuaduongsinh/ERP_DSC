...
export async function ClassRosterView(props: AdminViewServerProps) {
  ...
  const rawClass = Array.isArray(searchParams?.class) ? searchParams.class[0] : searchParams?.class
      ...
      <Gutter>
        <header style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: 'var(--theme-elevation-500)' }}>
            Xem học viên đang học của một lớp (trạng thái, số buổi còn lại, SĐT phụ huynh) và{' '}
            <strong>thêm / rút học viên</strong> khỏi lớp. Lọc theo cơ sở.
          </p>
        </header>
        {classes.length === 0 ? (
          <p style={{ color: 'var(--theme-elevation-500)' }}>
