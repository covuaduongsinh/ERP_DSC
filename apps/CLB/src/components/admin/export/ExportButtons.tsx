...
export function ExportButtons() {
  ...
  const handleExport = useCallback(
    async (format: Format) => {
      ...
      try {
        const response = await fetch(`/api/admin-export/${encodeURIComponent(collectionSlug)}`, {
          method: 'POST',
        ...
        const fileName =
          parseFileName(response.headers.get('Content-Disposition')) ?? `xuat.${format}`
    ...
    <div className="ds-export">
      <div className="ds-export__buttons">
        ...
        {typeof totalDocs === 'number' ? (
          <span className="ds-export__hint">Xuất {totalDocs} bản ghi theo bộ lọc hiện tại</span>
        ) : null}
      </div>
      {error ? (
