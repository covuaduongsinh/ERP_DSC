...
export function ApplyTemplatePanel({
  ...
  const onPreview = () => {
    ...
    startBusy(async () => {
      const res = await previewApplyTemplateAction({
        lopId,
        from,
        to,
        templateId,
  ...
  const onApply = () => {
    ...
    if (mode === 'overwrite' && !confirmOverwrite) {
      setMsg({
        kind: 'err',
        text: 'Chế độ ghi đè sẽ thay nội dung đã có — hãy tick xác nhận trước.',
      })
      return
    }
    startBusy(async () => {
  ...
  return (
    ...
    >
      <legend style={{ padding: '0 6px', fontWeight: 600 }}>
        Áp khung lộ trình (điền sẵn nội dung buổi)
      </legend>
          ...
          >
            ...
            <label className="ds-field" style={{ width: 130 }}>
              ...
              <input
                ...
                min={1}
                value={startIndex}
                onChange={(e) => setStartIndex(Math.max(1, Number(e.target.value) || 1))}
                disabled={busy}
              />
            </label>
            <label className="ds-field" style={{ width: 180 }}>
              ...
              <select
                ...
                value={mode}
                onChange={(e) => {
                  setMode(e.target.value as ApplyMode)
                  setConfirmOverwrite(false)
                }}
                disabled={busy}
              >
      ...
      {preview && preview.rows.length > 0 && (
        <div style={{ marginTop: 12 }}>
          <p style={{ margin: '0 0 8px', fontWeight: 600 }}>
            Xem trước “{preview.templateName}”: map <strong>{preview.sessionsMapped}</strong>/
            {preview.lessonsTotal} bài · sẽ điền <strong>{preview.toFill}</strong> ô, ghi đè{' '}
            <strong>{preview.toOverwrite}</strong> ô.
          </p>
          ...
          <div style={{ marginTop: 10 }}>
            <Button
              ...
              disabled={
                busy ||
                (preview.toFill === 0 && preview.toOverwrite === 0) ||
                (mode === 'overwrite' && !confirmOverwrite)
              }
              onClick={onApply}
            >
