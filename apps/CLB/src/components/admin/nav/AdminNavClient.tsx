...
import { LogOut, PanelLeftClose, ChevronRight, MapPin, type LucideIcon } from 'lucide-react'
      ...
      <nav className="ds-side ds-side--rail" aria-label="Điều hướng quản trị">
        ...
        <div className="ds-side__user ds-side__user--rail">
          <button
            ...
            aria-label="Mở rộng menu"
            style={{
              background: `linear-gradient(140deg, ${avatarColor}, var(--ds-primary))`,
              border: 'none',
              cursor: 'pointer',
            }}
            onClick={() => changeMode('full')}
      ...
      <div className="ds-side__brand">
        <span className="ds-side__mark">
          <Logo
            variant="symbol"
            color="#ffffff"
            style={{ width: 24, height: 26, display: 'block' }}
          />
        </span>
          ...
          return (
            <div
              className={
                'ds-navacc' +
                (open ? ' ds-navacc--open' : '') +
                (hasActive ? ' ds-navacc--has-active' : '')
              }
              key={g.group}
            >
              <button
