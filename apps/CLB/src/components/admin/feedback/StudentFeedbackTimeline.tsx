    ...
    <section className="ds-card" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {!editing ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {error && (
            <p role="alert" className="ds-error" style={{ margin: 0 }}>
              {error}
            </p>
          )}
          <div className="ds-formrow">
            ...
            <label className="ds-field">
              ...
              <input
                ...
                style={{ width: 80 }}
                value={form.yThuc}
                onChange={(e) => set('yThuc', e.target.value)}
                disabled={saving}
              />
            </label>
            <label className="ds-field">
              ...
              <input
                ...
                style={{ width: 80 }}
                value={form.lamBTVN}
                onChange={(e) => set('lamBTVN', e.target.value)}
                disabled={saving}
              />
            </label>
          </div>
          <label className="ds-field" style={{ display: 'block' }}>
            ...
            <textarea
              ...
              rows={2}
              value={form.nhanXet}
              onChange={(e) => set('nhanXet', e.target.value)}
              disabled={saving}
            />
          </label>
          <label className="ds-field" style={{ display: 'block' }}>
            ...
            <input
              className="ds-input ds-input--full"
              type="text"
              value={form.linkLichess}
              onChange={(e) => set('linkLichess', e.target.value)}
              disabled={saving}
            />
          </label>
...
export function StudentFeedbackTimeline({ data }: { data: TimelineData }) {
  const withFeedback = data.entries.filter(
    (e) => e.nhanXet || e.yThuc != null || e.lamBTVN != null,
  ).length
