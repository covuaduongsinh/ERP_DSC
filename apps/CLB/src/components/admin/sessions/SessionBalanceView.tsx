...
import { loadSessionBalances, loadStudentSessionBreakdown } from '@/lib/operations/session-balance'
...
export async function SessionBalanceView(props: AdminViewServerProps) {
  ...
  const studentId = parseStudentId(searchParams?.student as string | string[] | undefined)
      ...
      <>
        <header style={{ marginBottom: 16 }}>
          ...
          <p style={{ color: 'var(--theme-elevation-500)' }}>
            Số buổi tồn = số buổi đã nộp − số buổi đã học, theo từng học viên đang học. Mặc định
            tính theo <strong>lần nộp gần nhất</strong>; học viên đã chốt số dư đầu kỳ thì tính theo
            số dư đó. Bấm <strong>Chi tiết</strong> để xem cách đối trừ; dòng <strong>âm</strong> /{' '}
            <strong>thiếu phiếu thu</strong> cần đối soát.
          </p>
        </header>
