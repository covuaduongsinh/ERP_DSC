...
import { attendanceHref, sessionDetailHref } from '@/lib/operations/session-balance-links'
...
export function SessionBalanceClient({ rows }: SessionBalanceClientProps) {
  ...
  const stats = useMemo(
    () => ({
      ...
      reconcile: rows.filter((r) => r.flag === 'negative' || r.flag === 'none').length,
    }),
    [rows],
  )
                    ...
                    <tr>
                      ...
                      <td>
                        <a
                          ...
                          href={`/admin/collections/students/${r.studentId}`}
                        >
                          {r.studentName}
                        </a>
                      </td>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '4px 2px' }}>
      ...
      <div>
        ...
        <strong
          style={{
            color: row.balance !== null && row.balance < 0 ? 'var(--ds-critical)' : undefined,
          }}
        >
      ...
      {row.openingBalance !== null && row.openingDate && (
        <div>
          Số dư đầu kỳ (đến hết {fmtDate(row.openingDate)}):{' '}
          <strong>+{fmt(row.openingBalance)}</strong> buổi
        </div>
      )}
      <div>
