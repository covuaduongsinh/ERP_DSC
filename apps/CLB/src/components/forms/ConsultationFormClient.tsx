...
const fieldValid = 'border-line focus:border-navy focus:shadow-[0_0_0_3px_rgba(43,57,144,.14)]'
const fieldInvalid =
...
function validate(data: ConsultationFormData, tErrors: (key: string) => string): FieldErrors {
  ...
  else if (!isValidVietnamesePhone(data.phone)) errors.phone = tErrors('phoneInvalid')
...
export function ConsultationFormClient({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const data: ConsultationFormData = {
      ...
      interestedLocation: String(formData.get('interestedLocation') ?? '').trim() || undefined,
  ...
  return (
    <div className="grid overflow-hidden rounded-ds-2xl border border-line bg-white shadow-ds-lg min-[860px]:grid-cols-2">
      ...
      <div className="relative overflow-hidden bg-navy px-9 py-12 text-white sm:px-11">
        ...
        <h2 className="relative mt-3 text-[30px] font-black leading-[1.15]">{t('cardTitle')}</h2>
      ...
      <div className="p-9 sm:p-10">
        {status === 'success' ? (
          <div className="py-9 text-center" role="status" aria-live="polite">
            ...
            <h3 className="mb-2.5 text-[23px] font-black text-ink">{t('successTitle')}</h3>
          ...
          <form id={id} noValidate onSubmit={handleSubmit}>
            ...
            {status === 'error' && (
              ...
              >
                ...
                <p className="mt-1 text-muted">{serverErrorMessage || t('errorMessage')}</p>
              </div>
            )}
            ...
            <div className="mb-4">
              ...
              <input
                ...
                aria-describedby={fieldErrors.fullName ? `${id}-fullName-error` : undefined}
            ...
            <p className="mt-3.5 text-center text-xs text-muted">{t('privacyHint')}</p>
          </form>
        )}
      </div>
    </div>
