...
export function isTuitionLow(cycle: TuitionCycle): boolean {
  return cycle.status === 'sap_het' || sessionsRemaining(cycle) <= TUITION_LOW_THRESHOLD
}
 ...
 */
export function isTuitionCycleLowOrExpiring(cycle: TuitionCycle, now: Date = new Date()): boolean {
 ...
 */
export function suggestExpectedEndDate(startDate: string, pkg: TuitionCycle['package']): string {
  if (!startDate) return ''
