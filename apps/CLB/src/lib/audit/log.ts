...
function resolveActor(req: PayloadRequest | undefined): AuditActor {
  ...
  if (u.collection === 'users') {
    const label = u.email ? `${u.email}${u.role ? ` (${u.role})` : ''}` : `user:${u.id}`
 ...
 */
async function writeAudit(req: PayloadRequest | undefined, row: AuditRow): Promise<void> {
...
export function auditAfterChange(slug: AuditCollectionSlug): CollectionAfterChangeHook {
...
export function auditAfterDelete(slug: AuditCollectionSlug): CollectionAfterDeleteHook {
  const { fields } = AUDIT_CONFIG[slug]
  return async ({ req, id, doc }) => {
    const d = asRecord(doc)
    const actor = resolveActor(req)
    const documentId = id != null ? String(id) : d.id != null ? String(d.id) : null
