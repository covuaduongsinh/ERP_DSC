  ...
  try {
    ...
    if (!cls) {
      return {
        ok: false,
        error: 'forbidden',
        message: 'Không tìm thấy lớp (hoặc ngoài phạm vi cơ sở của bạn).',
      }
    }
...
export async function generatePlan(
  ...
  if (!isPlanner(actor)) {
    return {
      ok: false,
      error: 'forbidden',
      message: 'Chỉ quản lý hoặc admin mới được sinh buổi dự kiến.',
    }
  }
  if (!Number.isInteger(input.lopId) || input.lopId <= 0) {
    return { ok: false, error: 'invalid_input', message: 'Chưa chọn lớp.' }
...
export async function loadPlannedSessions(
  ...
  try {
    ...
    const rows: PlannedSessionRow[] = (docs as unknown as Array<Record<string, unknown>>).map(
      (d) => ({
...
export async function createPlannedMakeup(
  ...
  if (!isPlanner(actor)) {
    return {
      ok: false,
      error: 'forbidden',
      message: 'Chỉ quản lý hoặc admin mới được tạo buổi bù.',
    }
  }
