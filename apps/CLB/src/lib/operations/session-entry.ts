...
export async function loadSessionEntry(
  ...
  try {
    const session = await ensureSession(payload, actor, {
      lopId: input.classId,
      date: bounds.start,
    })
...
export async function saveSessionEntry(
  ...
  try {
    ...
    const currentTrangThai = String(
      (session as unknown as Record<string, unknown>).trangThai ?? 'du_kien',
    )
...
export async function setSessionCancelled(
  ...
  try {
    ...
    await payload.update({
      ...
      id: session.id,
      data: {
        trangThai: input.huy ? 'huy' : 'da_day',
        ghiChuBuoi: input.huy ? (input.reason ?? null) : null,
      },
      user: actor,
      overrideAccess: false,
    })
