...
export async function loadSessionList(payload: Payload, user: User): Promise<SessionListRow[]> {
  ...
  const classIds = Array.from(
    new Set(groups.map((g) => g.classId).filter((v): v is number => v !== null)),
  )
  const coachIds = Array.from(
    new Set(groups.map((g) => g.coachId).filter((v): v is number => v !== null)),
  )
  const sessionIds = Array.from(
    new Set(groups.map((g) => g.sessionId).filter((v): v is number => v !== null)),
  )
...
export async function loadSessionFeedback(
  ...
  const classDoc =
    ...
    (firstWithClass?.lop && typeof firstWithClass.lop === 'object'
      ? (firstWithClass.lop as Class)
      : null)
  ...
  const date = utcDayKey(session?.date) ?? utcDayKey(attendance[0]?.date) ?? null
...
export async function loadStudentFeedbackList(
  ...
  const agg = new Map<
    number,
    { sessionCount: number; feedbackCount: number; lastDate: string | null }
  >()
...
export async function updateAttendanceFeedback(
  ...
  try {
    ...
    if (wantsContent) {
      ...
      if (!current) {
        return {
          ok: false,
          error: 'forbidden',
          message: 'Không tìm thấy bản ghi (hoặc ngoài phạm vi cơ sở).',
        }
      }
    ...
    return {
      ok: false,
      error: 'server',
      message: 'Không lưu được (có thể ngoài phạm vi cơ sở của bạn).',
    }
  }
}
