...
export type SimpleTemplateResult = { ok: true } | { ok: false; error: ErrorCode; message: string }
...
export async function listTemplates(
  ...
  try {
    ...
    const templates: TemplateSummary[] = (docs as unknown as Array<Record<string, unknown>>).map(
      (d) => ({
        id: d.id as number,
...
export async function saveTemplate(
  ...
  if (!isCurriculumManager(actor)) {
    return {
      ok: false,
      error: 'forbidden',
      message: 'Chỉ quản lý hoặc admin mới được biên soạn khung lộ trình.',
    }
  }
...
export async function deleteTemplate(
  ...
  if (!isCurriculumManager(actor)) {
    return {
      ok: false,
      error: 'forbidden',
      message: 'Chỉ quản lý hoặc admin mới được xóa khung lộ trình.',
    }
  }
  if (!Number.isInteger(id) || id <= 0) {
    return { ok: false, error: 'invalid_input', message: 'Khung không hợp lệ.' }
...
export async function previewApplyTemplate(
  ...
  if (!isPlanner(actor)) {
    return {
      ok: false,
      error: 'forbidden',
      message: 'Chỉ quản lý hoặc admin mới được áp khung lộ trình.',
    }
  }
  try {
