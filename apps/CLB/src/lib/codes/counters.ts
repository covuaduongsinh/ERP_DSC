...
export function getCodeCounterPool(payload: { db: unknown }): CodeCounterPool {
  ...
  if (!pool) {
    throw new Error('Không truy cập được Postgres pool để cấp mã (payload.db.pool).')
  }
  return pool
}
 ...
 */
export async function nextSequence(pool: CodeCounterPool, scope: string): Promise<number> {
