 ...
 * `clb.payment.received` lên NATS để Accounting ghi nhận doanh thu (bút toán).
 *
 * Chỉ phát trên `create` (idempotent, tránh đăng bút toán trùng khi sửa). Lỗi
 * publish được nuốt trong publishClbEvent ⇒ không ảnh hưởng ghi Payments.
 */
export const publishPaymentReceived: CollectionAfterChangeHook = async ({
  ...
  const studentId = student && typeof student === 'object' ? student.id : student

  const data: TuitionPaymentReceived = {
    paymentId: String(doc.id),
    code: doc.code ? String(doc.code) : undefined,
    studentId: String(studentId ?? ''),
    paymentDate: doc.ngayNop ? new Date(doc.ngayNop).toISOString() : new Date().toISOString(),
  ...
  await publishClbEvent('clb.payment.received', data, { userId })
  return doc
}
