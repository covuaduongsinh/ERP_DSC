...
/**
 * Hooks phát sự kiện đồng bộ ngược CLB → CRM (view 360° trên Contact phụ huynh):
 *  - `clb.attendance.recorded`: mỗi lần điểm danh → cập nhật "hoạt động gần nhất".
 *  - `clb.student.updated`: học viên đổi cấp/trạng thái/số buổi tồn → cập nhật cache.
 *
 * Lỗi publish được nuốt trong publishClbEvent ⇒ không ảnh hưởng ghi dữ liệu.
 */
...
export const publishStudentUpdated: CollectionAfterChangeHook = async ({ doc, operation, req }) => {
  // Chỉ phát trên update — bản tạo mới đã đi qua handoff CRM→CLB.
