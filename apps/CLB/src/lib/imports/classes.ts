  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (seenInFile.has(dedupeKey)) {
      record({
        ...
        status: 'skipped',
        title,
        message: `Trùng Tên lớp "${title}" với dòng trước — bỏ qua.`,
      })
      continue
    }
    ...
    if (levels.length === 0) {
      record({
        row: rowNumber,
        status: 'error',
        message: `Cấp độ "${rawLevel}" không hợp lệ. Dùng: Tốt | Mã | Tượng | Xe | Hậu | Vua (phẩy ngăn nhiều cấp).`,
        field: 'cap_do',
      })
      continue
    }
    ...
    if (!rawAge) {
      record({
        row: rowNumber,
        status: 'error',
        message: 'Thiếu Nhóm tuổi (nhom_tuoi).',
        field: 'nhom_tuoi',
      })
      continue
    }
    ...
    if (ageGroups.length === 0) {
      record({
        row: rowNumber,
        status: 'error',
        message: `Nhóm tuổi "${rawAge}" không hợp lệ. Dùng: Mầm non | Cấp 1 - Cấp 2 | Cả hai.`,
        field: 'nhom_tuoi',
      })
      continue
    }
    ...
    if (!locationId) {
      record({
        row: rowNumber,
        status: 'error',
        message: `Cơ sở "${rawLoc}" không khớp danh sách Locations.`,
        field: 'co_so',
      })
      continue
    }
