  ...
  switch (kind) {
    ...
    case 'attendance':
      return normalizeAttendance(
        fileName,
        opts,
        await importNhanXetBuoi(payload, fileName, rows, opts),
      )
    case 'progress-reports':
      return normalizeProgressReports(
        fileName,
        opts,
        await importProgressReports(payload, fileName, rows, opts),
      )
    case 'coaches':
    ...
    case 'enrollments':
      return normalizeEnrollments(
        fileName,
        opts,
        await importEnrollments(payload, fileName, rows, opts),
      )
  }
}
...
function normalizeStudents(
  ...
  if (r.parentsCreated > 0)
    out.notes.push(
      `${r.parentsCreated} phụ huynh ${out.dryRun ? 'sẽ được tạo' : 'tạo mới'}; ${r.parentsLinked} lượt nối phụ huynh.`,
    )
...
function normalizeAttendance(
  ...
  if (r.coachLinkMissing > 0)
    out.notes.push(
      `${r.coachLinkMissing} bản ghi có GV nhưng chưa khớp Tên tắt (vẫn ghi, để trống GV).`,
    )
  return out
}
...
function normalizeProgressReports(
  ...
  out.notes.push(
    'Báo cáo nhập vào là NHÁP — phụ huynh chỉ thấy sau khi nhân viên điền Ngày phát hành.',
  )
  return out
}
...
function normalizeClasses(
  ...
  if (withWarning > 0)
    out.notes.push(
      `${withWarning} lớp có cảnh báo mềm (GV/lịch chưa khớp) — vẫn ghi, xem cột chi tiết.`,
    )
  return out
}
...
function normalizeEnrollments(
  ...
  out.notes.push(
    'Học viên khớp theo tên (trùng tên cần SĐT phụ huynh); lớp phải đã tồn tại (nhập Lớp trước).',
  )
  return out
}
