...
import { emptyResult, recordOutcome, type ImportResult, type NormalizedRow } from './types'
...
function normalizeStatusKey(input: string): string {
  return input.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().trim().replace(/\s+/g, '_')
}
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    try {
      const existing = await payload.find({
        ...
        where: {
          and: [{ student: { equals: lookup.studentId } }, { date: { equals: date } }],
        },
        limit: 1,
        overrideAccess: true,
        depth: 0,
