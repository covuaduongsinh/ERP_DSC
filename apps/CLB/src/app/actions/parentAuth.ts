...
export async function sendOtp(formData: FormData): Promise<SendOtpResult> {
  ...
  if (!isParentSessionSecretConfigured()) {
    return {
      success: false,
      error: 'server',
      message: 'Hệ thống chưa được cấu hình đủ (PARENT_SESSION_SECRET). Liên hệ quản trị.',
    }
  }
  ...
  try {
    ...
    if (found.totalDocs === 0) {
      ...
      return {
        success: false,
        error: 'unknownPhone',
        message: 'Không tìm thấy phụ huynh ứng với số này. Liên hệ trung tâm để được hỗ trợ.',
      }
    }
    ...
    return provider.exposesCodeToClient ? { success: true, devCode: code } : { success: true }
  } catch (err) {
