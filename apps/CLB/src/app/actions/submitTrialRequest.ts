...
// ---------------------------------------------------------------------------
export async function submitTrialRequest(formData: FormData): Promise<SubmitTrialRequestResult> {
  ...
  try {
    ...
    const locationId =
      typeof openClass.location === 'number' ? openClass.location : (openClass.location?.id ?? null)
