...
import { isTuitionLow, sessionsRemaining, toParentPayloadUser } from '@/lib/parent-portal'
...
export async function submitRenewalRequest(
  ...
  try {
    ...
    const tuitionCycleId = tuitionCycleIdRaw ? Number.parseInt(tuitionCycleIdRaw, 10) : null
    ...
    if (!tuitionCycle) {
      const cycles = await payload.find({
        ...
        overrideAccess: false,
        where: {
          and: [{ student: { equals: studentId } }, { status: { in: ['dang_hoc', 'sap_het'] } }],
        },
        sort: '-startDate',
        limit: 1,
        depth: 0,
    ...
    const existing = await payload.find({
      ...
      overrideAccess: false,
      where: {
        and: [{ student: { equals: studentId } }, { status: { in: ['moi', 'dang_xu_ly'] } }],
      },
      limit: 1,
      depth: 0,
    })
    ...
    if (existing.totalDocs > 0) {
      return {
        success: false,
        error: 'duplicate',
        message: 'Bạn đã gửi yêu cầu gia hạn. Nhân viên sẽ liên hệ trong thời gian sớm nhất.',
      }
    }
    ...
    return {
      success: true,
      message: 'Đã gửi yêu cầu gia hạn. Nhân viên sẽ liên hệ để tư vấn và chốt gói mới.',
    }
  } catch (err) {
