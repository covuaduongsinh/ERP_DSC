...
import { CLASS_CODE_SPEC, EVENT_CODE_SPEC, BOOK_ISSUE_CODE_SPEC } from '@/lib/codes/field'
 ...
 */
function makeFakeReq(pool: CodeCounterPool, locations: Record<number, string | undefined> = {}) {
...
describe('Phase 3 Classes — thiếu/không phân giải được cơ sở → throw', () => {
  ...
  it('Classes: location trỏ tới cơ sở CHƯA có code → throw', async () => {
    ...
    await expect(runHook(CLASS_CODE_SPEC, req, { location: 9 })).rejects.toThrow(/cơ sở/i)
  })
  ...
  it('Classes: cache theo request — create TUẦN TỰ cùng location chỉ findByID 1 lần', async () => {
    ...
    const pool = makeFakePool()
    const req = makeFakeReq(pool, { 7: 'KL' })
    const datas = Array.from({ length: 5 }, () => ({ location: 7 })) as Record<string, unknown>[]
    ...
    const seqs = datas.map((d) => Number((d.code as string).split('-')[2])).sort((a, b) => a - b)
...
describe('Phase 3 bất biến — lockCodeHook chặn sửa code mọi vai trò', () => {
  ...
  it('Classes: update đổi code (kể cả admin) → throw', () => {
    expect(() => run({ code: 'LOP-KL-999' }, { code: 'LOP-KL-001' })).toThrow(/bất biến/i)
  })
