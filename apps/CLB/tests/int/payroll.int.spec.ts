...
describe('computePayroll — số buổi distinct (ngày+lớp) × đơn giá', () => {
  ...
  it('GV chưa set lương ⇒ thành tiền 0 (luongMoiBuoi null)', () => {
    const b = computePayroll([{ coachId: 2, date: '2026-06-01T08:00:00Z', lop: 5 }], coaches).find(
      (r) => r.coachId === 2,
    )!
  ...
  it('coachId null bị bỏ qua', () => {
    const rows = computePayroll([{ coachId: null, date: '2026-06-01T08:00:00Z', lop: 1 }], coaches)
    expect(rows.every((r) => r.soBuoi === 0)).toBe(true)
  })

  it('ngày sai định dạng bị bỏ qua', () => {
    const a = computePayroll([{ coachId: 1, date: 'không-phải-ngày', lop: 10 }], coaches).find(
      (r) => r.coachId === 1,
    )!
