...
import { groupOpenClassesByTrack, type OpenClassCard } from '@/lib/open-classes'
...
describe('groupOpenClassesByTrack', () => {
  it('gom theo tuyến, thứ tự theo trackOrder bất kể input', () => {
    const groups = groupOpenClassesByTrack([card(1, 'vua'), card(2, 'tot'), card(3, 'ma')])
    expect(groups.map((g) => g.track)).toEqual(['nhap_mon', 'co_ban', 'nang_cao'])
  })

  it('trong tuyến sắp theo levelOrder (Mã → Tượng → Xe)', () => {
    const groups = groupOpenClassesByTrack([card(1, 'xe'), card(2, 'ma'), card(3, 'tuong')])
